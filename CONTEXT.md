...
**2026-07-22 整改与产品化执行入口**：P0 使用 `docs/specs/beauty-marketing-agent-p0-remediation-spec-2026-07-22.md`，P1 使用 `docs/specs/beauty-marketing-agent-p1-productization-spec-2026-07-22.md`；两者取代历史 `beauty-content-agent-p0/p1-spec.md` 作为当前整改/产品化实施入口，但不抹除历史证据。Composer 营销 Task 使用 `CreationExecutionSnapshot + DBOS Harness`。**Pro Studio 已退役（D-170）**：不以 D-099 / parity rework / ADR-0012 为现行重做或升单入口；退役实施＝`docs/specs/pro-studio-retirement-spec-2026-08-01.md`。共享账本、对象存储、Capability、OwnedAsset 与 ContentPackage 写入不变量仍由 P0/P1 当前 Spec 裁决。历史 advancedCanvas 血缘字段只读，禁止新 adoption 写入。

**2026-07-24 决策批次覆盖**：设计文档决策日志已续至 **D-132**（D-110 元原则＝新批次优先于更早决策与旧口径）。要点：D-102 单店单人首发（Owner/Operator/Reviewer 降为内部 RBAC，前台只呈现当前账号本人）；D-103/D-111/D-114 定制创作＝agent 流式对话主容器＋自由创作参数面板双主线；D-105 FFmpeg 创意拼装退出视频主链（交付终点＝模型原生 VideoArtifact）；D-113 默认 1 主候选；D-121 内容工作区（账号 1:N、与租户严格分离、套餐 1/2/3）；D-123 文案/图片/视频三桶额度＋三类加油包（档位＝初级/中级/高级，数字＝运营后台手填）；D-126 Dashboard 首页轻推荐＋Day-0 示例店。本 Context 词条未逐条同步处，以设计文档决策日志为准。同日新增 **ADR-0013～0019** 七个实施投影 ADR（五段 Harness 与四类编译器/双主线交互/ContentWorkspace 租户分离/计费三桶四账本/解析管线四层/生成期零阻断发布收口/纵切交付与接缝属主）——实施 Agent 优先读这七个短 ADR 获取架构合同，细节与冲突裁决回决策日志。**D-127 开发路线**：原地换壳保核——存量代码分桶处置（KEEP+EXTEND 骨架／RETIRE 真删：compose/烧录链·槽位残留·换壳后旧 IA 页／**Pro Studio 原 FREEZE 行已由 D-170 升格为 RETIRE 全量退役**——见 `docs/specs/pro-studio-retirement-spec-2026-08-01.md`／REBUILD：前端沿 Composer 主干换壳、旧面整批下线无双轨期／NEW：解析管线·四类编译器·三桶扩展·首页挂载）；god module 触碰时拆；**每张票动工前先归桶——禁止在 RETIRE 桶续命、在 REBUILD 桶打补丁**。**D-128 拆票前四口径**：v1 变体平台＝小红书/抖音/视频号，朋友圈＝distributionTarget（导出＋assisted，不做变体主体）；注册试点期＝邮箱＋兑换码＋运营代开，手机号短信挂 E 门；产品命名挂 R 门前专项；示例任务产物允许导出、判断同正式使用，试用状态额度＝后台人工动态开关。**D-129 组件选型收口**：MinerU＝官方精准 API、数据出域＝温柔文字免责不设门槛（自托管挂 E 门同批）；文案与编排判断位默认 LLM＝DeepSeek（默认型号＝deepseek-v4-pro、v4-flash 轻档备选 catalog 可换、需视觉判断位除外，官方文档全站镜像 `references/analysis/deepseek-api-docs-2026-07-24/` 实施票只引镜像）；基础组件优先以 MkFast/Cloudflare 既有能力为实现基础——邮件＝模板内置 Resend、对象存储＝R2；其余栈决策逐项扫描均闭合，实施 Agent 无选型临场发明空间；同日补充：图/视频默认型号＝seedream-5-pro/seedance-2 系·通道默认＝方舟直连（tuzi relay 容灾）、exactText 校验＝既有多模态 VLM 零新件、Recipe/Skills 载体＝PG＋提示词 Langfuse；后台表单渲染组件悬置已由 D-130 解除。**D-130 前端组件供给基准＝HeroUI Pro V3**（用户专属授权仓 Agile-Avocation/herouipro-v3，本地镜像 `references/repos/herouipro-v3/` gitignore 不入库不分发）：`@ag-ui/pro` beta.7＝HeroUI v3＋Tailwind v4＋React 19＋React Aria，与现栈（React 19.2/Tailwind 4.1.18）零升级对齐；REBUILD 换壳与 NEW 净新建面优先取用，AI Chat 组件族（chat-conversation/prompt-input/chain-of-thought 等）正对 D-114 对话流主容器，D-107 后台动态表单＝其表单组件族组装（不引 JSON-Schema 表单框架）；KEEP 桶不为换库专项迁移（触碰时换）；「门店橱窗」DESIGN.md 仍为唯一视觉权威，组件经 token 适配接入（主题拍板＝Glass）；**Landing 页例外**＝保持现状仅改文案与前后链接（专项优化随 D-125 阶段二），其余页面组件与动效均优先取用；模板起点＝template-chat→工作区交互页、template-dashboard→运营后台可视化。**D-131 拆票组织**：前端相关票独立成组（frontend lane），由用户指定 Agent 与模型执行（已登记＝Claude Opus）；分组不破 ADR-0019 纵切——接缝合同与属主显式写明、旅程收口落前端票或联调票、组内自闭环不接真后端＝未完成；前端组票面自包含（DESIGN.md＋ADR-0014＋D-130 上下文包）。**归桶判定矩阵已终裁并经用户终审批准（2026-07-25）**：`docs/specs/bucket-disposition-matrix-2026-07-25.md`（223 行：KEEP 156＋mount-pending 2／RETIRE 48 四批获准——1A 立即删 36 行可开退役票、1B/1C/1D 挂条件门／REBUILD 10／FREEZE 7／UNCERTAIN 0）；拆票以此为地基，**每张票动工前先查表**。**D-132 凭证与运营供给单门（2026-07-25）**：凭证项（模型/渠道/解析/邮件/存储 key 与账号开通）与运营项（三桶与加油包数字、试用开关、兑换码规则、示例店素材、行业先验、对外名称）归并单一供给门类——开票前统一供给清单 `docs/ops/provisioning-manifest.md` 由用户一次性补齐，票面只按清单键引用不得单独索取；开发与 CI 默认档一律 fixture/种子值走绿（缺凭证不阻塞不假红），真凭证到位后 live_verified 门一次核销（核销归供给门票不进功能票 DoD）；运营数字以种子样例值验收、真值运营期后台手填。开发规格已发布：`docs/specs/reshell-and-extend-dev-spec-2026-07-25.md`＝GH issue #194（ready-for-agent，经三路 Codex 交叉审核修复后 v2），拆票唯一输入＝该 spec＋归桶矩阵＋供给清单。**票包已发布（2026-07-25）**：40 票＝GH **#195–#234**（issue 号＝194＋票号；依赖序发布，Blocked-by 为真实 issue 号；FE 九票带 frontend-lane 标签，执行 Agent＝Opus）；流水线＝5 fork 起草→4 路 Codex 复核（2 P0+29 P1+6 P2）→3 fork 修订全清；票体镜像 `.scratch/tickets-reshell-2026-07-25/`＋issue-numbers.json。前沿票＝#195/#196/#197（无阻塞）。供给清单已全量补齐（2026-07-25：A-5 MinerU 入本地 env、B-1 发信域名＝tqai.uk，DNS 验证随 #199）。
...
**2026-08-08 Agent-native V3.1 批次覆盖**：决策日志已续至 **D-178**；agent-native 升级的实施与架构权威＝`docs/design/0808规划/meiye-agent-v3.1-authoritative-plan-2026-08-08.md`（V3.1：44 节＋附录 A 19 条硬约束＋附录 B **U1–U14** 十四项拍板＋附录 C 框架引用来源；三轮评审与九 spec 复核报告在 `docs/reviews/` 同日四份）。实施投影＝**ADR-0020**（ADR-0007/0013/0014 头部已加修订注：Session Harness 增补／五段收敛为 trace taxonomy／主容器升级 Thread-root Workstream）。术语：**双 Harness**＝Agent Session Harness（AI SDK streamText 工具环，只读优先）＋ Production Make Harness（DBOS durable，付费副作用），交接物唯一＝冻结 `ExecutionPlanSnapshot`（`approvalBasis: merchant_confirmed | policy_exempt_copy`，纯 copy 免确认**不免冻结**）；**plan-as-data**＝`CompiledExecutionPlan`（typed unit＋依赖分组＋重试默认关，控制流留 DBOS TS 代码，禁 grammar 解释器/任意 DAG）；**AgentThread**＝跨 Work 长期会话一等对象（Thread-root Workstream 文档时间线，「最近」页收编为 Thread 列表投影，supersede D-088）；**Progressive Plan Level 0–3**（L0 确定性轻修改／L1 纯 copy 免确认永久口径／L2 Living Plan／L3 Campaign 每付费 Work 单独确认）；**Memory 平台**＝五层认知分类＋双通道 authority＋生命周期分置存储＋注入透明（MemoryInjectionReceipt）；**HarnessRelease 三对象**＝immutable Artifact/Lifecycle/Rollout（per-run 试跑只选完整 candidate releaseId，闸门数值 unset 拒进生产）。**实施票编号 #1–#9，票面真相＝本地文件**（依赖顺序 A 地基→B 会话方案→C 确认执行→D 交付→E Memory 并行 lane→F Goal/Proactive→G Release/Eval→H 运营控制面→I Legacy 退役；本地快照 `docs/specs/v3.1-agent-specs-2026-08-08/`＋`docs/specs/v3.1-spec-{H,I}-*.md`）。GitHub 两账号（leelv007-cmd／leelv008）2026-08-08 均封禁待申诉——leelv008/meiyeagent 曾发布 #1–#7 后遭关联封禁；恢复前**不做任何 GitHub 写操作、不开新账号**，开发票同样落本地（`docs/tickets/v3.1/`）。原 leelv007-cmd/meiyeweb-agent #429–#435 **废弃不再引用**。过渡期新旧链共存，退役门＝V3.1 §35 批次 6。
...
**开放图文工作台**:
The P1 Composer capability for daily light editing: template rendering, text/image/crop/order changes, custom template reuse, preview, save revision, export, and AI image generation or editing available during authoring. The product has **no** infinite canvas workspace, professional post-production surface, or canvas Agent. Product-brand watermark and AIGC-label controls are exposed as switches, with no legal/compliance check gating development or draft creation.
_Avoid_: 约束式 renderer, 有限编辑器, 用发布合规限制创作功能, 把日常轻编辑说成属已退役 Pro Studio 加购

**Composer 日常轻编辑**:
The mainline P1 editing surface inside 创作. It is intentionally bounded to fast, repeatable merchant operations and must not silently grow into an infinite canvas or professional post-production surface. Free creation remains a model-native thin path (not a canvas).
_Avoid_: P1 默认无限画布, 专业精修混入主线, 恢复已退役 Pro Studio 入口或加购分界

**Pro Studio（已退役）**:
Historical workspace add-on for infinite canvas, precision editing, TTS/SFX, and online canvas Agent. **Fully retired (D-170, 2026-08-01)** — no new development, sale, product entry, or adoption write path; no canvas successor. Free creation = model-native thin path only. Previously adopted ContentPackages remain read-only. Implementation authority: `docs/specs/pro-studio-retirement-spec-2026-08-01.md`. Not the P1 Composer capability and not the meaning of 高用量 Pro / 高级 plan tiers.
_Avoid_: 当现行能力, 恢复 FREEZE 只留入口, 并行工程票 / parity K1–K7 现行入口, `/pro-studio` 必达, 新 advancedCanvas adoption, 半死垫片, 把套餐「Pro」档位当成画布加购
...
**画布作品**:
An editable page-based Composer design document pinned to its template version and referencing Assets. Inserting a generated Asset creates a new Work revision; exporting creates an Asset only after the exported binary is durably persisted. A Work is neither the exported media nor the publishable ContentPackage. Historical Pro Studio `AdvancedCanvasProject` (retired, D-170) was a separate graph/revision aggregate; leave any existing `advancedCanvas` lineage read-only — no new adoption write path.
