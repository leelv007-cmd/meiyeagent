...
暗色是主应用的正式主题（D-042 决策一），不是模板暗色继承——同一套「门店橱窗」范式换材质在暗色下重演：氛围层仍是同一张渐变底图（换纱不换图），白瓷与玻璃只是把明度反过来。本节codify已落地的暗色对应物（`.dark .meiye-product-shell`），Pro Studio 的独立深色影棚不受本节约束。
