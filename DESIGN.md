...
- 阴影近乎为零，深度靠磨砂玻璃层次表达
- 双主题正式（浅色 + 暗色均为正式主题，D-042）；Pro Studio 保持独立深色影棚密度
...
暗色是主应用的正式主题（D-042 决策一），不是模板暗色继承——同一套「门店橱窗」范式换材质在暗色下重演：内容影像仍做氛围层，白瓷与玻璃只是把明度反过来。本节codify已落地的暗色对应物（`.dark .meiye-product-shell`），Pro Studio 的独立深色影棚不受本节约束。
...
### 氛围层与遮罩
- 氛围图不变，压暗遮罩加深：`ambient-scrim-top = oklch(0 0 0 / 0.72)`、`ambient-scrim-mid = oklch(0 0 0 / 0.5)`（浅色为 0.62 / 0.28）。
- **压字白色不变**：`ambient-text = oklch(1 0 0 / 0.96)` 两套主题同值。问候语与所有浮在氛围顶带上的页头文字（`.meiye-ambient-copy` 容器：门店/内容库/素材/会话等内页页头、移动工作台页头）在明暗两态都实测 ≥4.5:1。落在实底白瓷区（如 bg-surface-0 内容区）的页头仍用墨色梯度，不套 ambient-copy。
