...
test("revalidates the frozen read-set then calls only Core quote and submit", async () => {
	...
	const core = {
		...
		async quote(input: CoreGenerationQuoteInput) {
			...
			assert.deepEqual(input.inputNodeBindings, [
				{
					assetId: "asset-1",
	...
	projectNodes.push({
		data: { assetId: "asset-1" },
		id: "image-reference-duplicate",
		kind: "image",
	});
	await assert.rejects(
		adapter.quote(context, input),
		/Canvas Agent generation input node is unavailable or ambiguous/u,
	);
	assert.deepEqual(calls, ["quote", "submit"]);
});
