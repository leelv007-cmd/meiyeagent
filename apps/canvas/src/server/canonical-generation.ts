...
async function generationRequest(
	...
	projects: Pick<CanvasAgentRepository, "readGraph">,
): Promise<CoreGenerationQuoteInput> {
	const graph = await projects.readGraph(context.workspaceId, input.projectId);
	if (!graph) {
		throw new CanonicalGenerationError(
			"AGENT_GENERATION_READ_SET_UNAVAILABLE",
			"Canvas Agent generation graph is unavailable.",
			{ retryable: false },
		);
	}
	const inputNodeBindings = input.inputAssets.map((asset) => {
		const expectedKind =
		...
		const matchingNodes = graph.nodes.filter(
			(candidate) =>
				...
				candidate.data.assetId === asset.assetId,
		);
		if (matchingNodes.length !== 1) {
			throw new CanonicalGenerationError(
				"AGENT_GENERATION_READ_SET_UNAVAILABLE",
				"Canvas Agent generation input node is unavailable or ambiguous.",
				{ retryable: false },
			);
		}
		const [node] = matchingNodes;
		if (!node) throw new Error("Unreachable input node validation state.");
