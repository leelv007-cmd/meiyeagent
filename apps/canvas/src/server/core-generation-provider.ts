...
export class CoreGenerationProvider {
	...
	constructor(private readonly options: CoreGenerationProviderOptions) {
	...
	private async request(
		...
		const headers: Record<string, string> = {
			...
			"x-core-actor": "worker",
			"x-correlation-id": requireText(identity.correlationId, "correlationId"),
			"x-service-token": this.options.coreServiceToken,
			"x-user-id": requireText(identity.userId, "userId"),
		...
		try {
			response = await this.fetcher(
				new URL(
					`/v1/workspaces/${encodeURIComponent(workspaceId)}/p1/${kind}`,
					this.serviceUrl,
				),
				{
			...
			throw new CoreGenerationProviderError(
				"CORE_UNREACHABLE",
				`Core generation request failed: ${errorMessage(error)}.`,
				{ retryable: true },
			);
		}
		...
		if (!response.ok) {
			const remote = remoteError(envelope);
			throw new CoreGenerationProviderError(
				remote.code ?? "CORE_GENERATION_REJECTED",
				remote.message ??
					`Core generation request failed with status ${response.status}.`,
				{
...
function coreProviderInput(input: CoreGenerationQuoteInput) {
	...
	for (const key of ["maxOutputTokens"] as const) {
		...
		if (value === undefined) continue;
		if (!Number.isSafeInteger(value) || (value as number) <= 0) {
			throw new CoreGenerationProviderError("CORE_PARAMETER_INVALID", `${key} must be a positive integer.`, { retryable: false });
		}
		parameters[key] = value as number;
	}
	if (input.parameters.temperature !== undefined) {
		...
		if (typeof value !== "number" || !Number.isFinite(value)) {
			throw new CoreGenerationProviderError("CORE_PARAMETER_INVALID", "temperature must be a finite number.", { retryable: false });
		}
	...
	for (const key of ["strength", "speed", "maxDurationSeconds"] as const) {
		...
		if (
			...
			throw new CoreGenerationProviderError("CORE_PARAMETER_INVALID", `${key} must be a valid number.`, { retryable: false });
		}
		parameters[key] = value;
	}
	...
	return {
		inputAssets: [...new Map(inputAssets.map((asset) => [`${asset.role}:${asset.assetId}`, asset])).values()],
		parameters,
	};
}
