...
export function CanvasNodeHoverToolbar({
	...
	useEffect(() => {
		void node?.id;
		void quickImageToolIds;
		void showImageToolLabels;
		void imageToolSettingsOpen;
	...
	const safeToolbarWidth = Math.min(
		toolbarMetrics.width || 0,
		Math.max(0, safeViewportWidth - 32),
	);
	const toolbarLeft =
		safeViewportWidth && safeToolbarWidth
			? Math.min(
					Math.max(left, safeToolbarWidth / 2 + 16),
					safeViewportWidth - safeToolbarWidth / 2 - 16,
				)
			: left;
...
export function CanvasNodeInfoModal({
	...
	const batchCount =
		node?.type === CanvasNodeType.Image
			? node.metadata?.batchChildIds?.length || 0
			: 0;
	const json = useMemo(() => {
		if (!node) return "";
				...
				<div className="h-[56vh] min-h-[360px] text-sm">
					{view === "info" ? (
						<div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
							...
							{batchCount > 1 ? (
								<InfoRow label="图片组" value={`${batchCount} 张`} />
							) : null}
							{node.metadata?.prompt ? (
								<InfoRow label="提示词" value={node.metadata.prompt} />
							) : null}
							{imageBytes ? (
								<InfoRow label="图片大小" value={formatBytes(imageBytes)} />
							) : null}
