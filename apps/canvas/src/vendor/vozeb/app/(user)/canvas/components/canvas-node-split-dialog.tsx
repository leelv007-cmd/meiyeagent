...
export function CanvasNodeSplitDialog({
	...
	const update = (
		key: keyof CanvasImageSplitParams,
		value: string | number | null,
	) => {
		setParams((current) => ({
			...current,
			[key]: clampGrid(value ?? current[key]),
		}));
	};
...
function SplitGrid({ rows, columns }: CanvasImageSplitParams) {
	return (
		<div className="pointer-events-none absolute inset-0">
			{dividerPositions(columns).map((position) => (
				<div
					key={`column-${position}`}
			...
			{dividerPositions(rows).map((position) => (
				<div
					key={`row-${position}`}
...
function dividerPositions(count: number) {
	const positions: number[] = [];
	for (let divider = 1; divider < count; divider += 1) {
		positions.push(divider / count);
	}
	return positions;
}
