...
import { type CanvasNodeData, CanvasNodeType, type Position } from "../types";
...
export const CanvasNode = React.memo(function CanvasNode({
	...
	const isBatchRoot =
		...
		Boolean(data.metadata?.isBatchRoot) &&
		batchCount > 1;
	const isBatchChild =
		data.type === CanvasNodeType.Image && Boolean(data.metadata?.batchRootId);
	...
	return (
		<fieldset
			aria-label={data.title || "画布节点"}
			data-node-id={data.id}
			className={`node-element absolute m-0 flex min-w-0 select-none flex-col border-0 p-0 transition-shadow duration-200 ${isSelected ? "z-50" : "z-10"}`}
			style={{
		...
		>
			<fieldset
				className="relative m-0 h-full min-w-0 w-full overflow-visible rounded-3xl border-2 p-0"
				style={{
					background:
			...
			>
				...
				{resourceLabel ? (
					<ResourceLabelBadge reference={resourceLabel} />
				) : null}
			...
			<ConnectionHandleDot
				...
				onConnectStart={(event) => onConnectStart(event, data.id, "target")}
			/>
			<ConnectionHandleDot
				side="right"
				visible={
				...
				onConnectStart={(event) => onConnectStart(event, data.id, "source")}
			/>
...
function NodeContent(props: NodeContentRendererProps) {
	...
	return Renderer ? (
		...
		<UnknownNodeContent theme={props.theme} />
	);
}
...
function UnknownNodeContent({
	theme,
}: Pick<NodeContentRendererProps, "theme">) {
	return (
		<div
...
function ResourceLabelBadge({
	...
	return (
		<span
			className={`pointer-events-none absolute right-2 top-0 z-[80] -translate-y-[calc(100%+6px)] rounded-md px-1.5 py-0.5 text-[10px] font-medium ${reference.active ? "bg-[#2f80ff] text-white shadow-sm" : "bg-black/35 text-white/75"}`}
		>
			{reference.label}
		</span>
	);
		...
		>
			...
			{isBatchRoot ? (
				...
				>
					...
					<ChevronRight
						className={`size-3.5 transition-transform ${batchExpanded ? "rotate-90" : ""}`}
...
function BatchFrame({
	...
	return (
		<fieldset
			className="group/batch relative m-0 h-full min-w-0 w-full overflow-visible border-0 p-0"
			onDoubleClick={
				isBatchRoot
					? (event) => {
							event.stopPropagation();
		...
		>
			{isBatchRoot ? (
				<div className="pointer-events-none absolute inset-0 overflow-visible">
					{batchLayers(batchCount).map((layer) => (
						<div
							key={layer.id}
							...
							style={{
								...
								transform:
									batchOpening || batchRecovering
										? `translate(${54 + layer.index * 22}px, ${20 + layer.index * 12}px) rotate(${8 + layer.index * 5}deg) scale(.98)`
										: `translate(${34 + layer.index * 18}px, ${14 + layer.index * 10}px) rotate(${6 + layer.index * 4}deg)`,
...
function batchLayers(batchCount: number) {
	const layers: Array<{ id: string; index: number }> = [];
	for (let index = 0; index < Math.min(batchCount - 1, 5); index += 1) {
		layers.push({ id: `batch-layer-${index + 1}`, index });
	}
	return layers;
}
...
function ResizeHandle({
	...
	return (
		<button
			type="button"
			tabIndex={-1}
			aria-label={`调整节点尺寸：${corner}`}
			className={`absolute z-50 size-7 border-0 bg-transparent p-0 ${positionClass}`}
...
function ConnectionHandleDot({
	...
	side: "left" | "right";
	visible: boolean;
	onConnectStart: (event: React.MouseEvent | React.PointerEvent) => void;
}) {
	const theme = canvasThemes[useThemeStore((state) => state.theme)];
	...
	return (
		<button
			type="button"
			tabIndex={-1}
			aria-label={`${side === "left" ? "输入" : "输出"}连线点`}
