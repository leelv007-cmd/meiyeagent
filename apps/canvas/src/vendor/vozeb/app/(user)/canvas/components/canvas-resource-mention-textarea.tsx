...
>(function CanvasResourceMentionTextarea(
	...
	const hasActiveLabelInValue = activeLabels.some((label) =>
		value.includes(label),
	);
	const showOverlay = Boolean(
		value && hasActiveLabelInValue && !hasSelection && !focused,
	);
	const mergedStyle = {
		...(style || {}),
...
function MentionHighlightText({
	...
	const parts = highlightedParts(value, pattern);
	return (
		<>
			{parts.map(({ id, text }) =>
...
function highlightedParts(value: string, pattern: RegExp) {
	let offset = 0;
	return value.split(pattern).map((text) => {
		const start = offset;
		offset += text.length;
		return { id: `${start}:${offset}:${text}`, text };
	});
}
