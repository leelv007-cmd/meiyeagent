...
export function VozebCanvas({
	...
	const readTouchPair = () =>
		Array.from(touchPointersRef.current.values()).slice(0, 2);
	...
	const updatePinchZoom = useCallback(
		(event: PointerEvent) => {
			...
			if (!pinchStateRef.current.active || touchPointersRef.current.size < 2)
				return false;
			...
			const newScale = Math.min(
				Math.max(
					pinchStateRef.current.startScale * (distance / startDistance),
					0.05,
				),
				5,
			);
		...
		[onViewportChange, containerRef.current?.getBoundingClientRect],
	);
	...
	}, [onCanvasDeselect, onViewportChange, updatePinchZoom]);
