...
export function buildCanvasResourceReferences(
	...
	const contextNodes = contextNodeId
		...
		: [];
	const globalReferences = labelResourceNodes(
		nodes.filter(isResourceNode),
		false,
	);
	const activeByNodeId = new Map(
		labelResourceNodes(contextNodes, true).map((reference) => [
			reference.nodeId,
			reference,
		]),
	);
	return globalReferences.map(
		(reference) => activeByNodeId.get(reference.nodeId) || reference,
	);
}
