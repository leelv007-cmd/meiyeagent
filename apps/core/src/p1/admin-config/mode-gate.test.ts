...
test('blocks new provider execution within the five-second disabled gate window', async () => {
  let now = 0;
  let effects = 0;
...
test('keeps the last known disabled state when the config head cannot be read', async () => {
  let now = 0;
  let failReads = false;
  let effects = 0;
  const inner: ProviderExecutionPort = {
