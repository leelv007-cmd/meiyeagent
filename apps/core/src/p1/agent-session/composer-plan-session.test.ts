...
test('viral adapt auto-starts Make after the source confirm so 图文方向 can park', async () => {
  const sessions = new MemoryAgentSessionStore();
  const plans = new MemoryMarketingPlanStore();
  const compiler = new PlanCompiler({
    store: plans,
  ...
  const coordinator = new ComposerPlanSessionCoordinator(sessions, plans, {
    ...
    async runComposerTurn() {
      return {
        decision: {
          merchantMessage: '已识别为爆款复刻图文计划',
          action: {
            kind: 'propose_plan',
            proposal: {
              goalNarrative: '按已确认参考笔记复刻本店图文',
              recommendedDeliverables: [
                { carrier: 'note', platform: 'xiaohongshu', quantity: 3 },
              ],
            },
  ...
  submission.snapshot = {
    ...
    viralAdaptSource: {
      ...
      authorizedAssetIds: ['asset-case-1'],
    },
  };
  const binding = await coordinator.prepare({ submission });

  // Ordinary paid notes stay makeReady false (Living Plan 开始制作). Viral
  // already collected source confirm; Make must start so the in-execution
  // 两种图文方向 ask can park on the merchant-visible conversation slot.
  assert.equal(binding.makeReady, true);
  assert.equal(
