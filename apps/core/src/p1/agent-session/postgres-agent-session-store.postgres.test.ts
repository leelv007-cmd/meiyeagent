...
test(
  'Postgres replays a crash-window handoff into one durable execution',
  { skip },
  async () => {
    const pool = new Pool({ connectionString });
    const store = new PostgresAgentSessionStore(pool);
    ...
    try {
      ...
      const handoffs = await Promise.all([
        store.linkExecutionRun({
          ...
          runId: `${resourceId}-child-a`,
          workflowId: 'workflow-paid',
          snapshotHash: 'snapshot-hash-frozen',
          now: '2026-08-08T13:02:00.000Z',
        }),
        store.linkExecutionRun({
          resourceId,
          parentRunId: `${resourceId}-turn`,
          runId: `${resourceId}-child-b`,
          workflowId: 'workflow-paid',
          snapshotHash: 'snapshot-hash-frozen',
          now: '2026-08-08T13:02:00.500Z',
        }),
      ]);

      assert.deepEqual(
        handoffs.map((handoff) => handoff.replayed).sort(),
        [false, true],
      );
      assert.equal(
        new Set(handoffs.map((handoff) => handoff.run.runId)).size,
        1,
      );
      const runs = await store.listRuns({ resourceId, threadId });
      const children = runs.filter((run) => run.durability === 'sync');
      assert.equal(children.length, 1);
      assert.deepEqual(children[0]?.executionLink, {
        workflowId: 'workflow-paid',
        snapshotHash: 'snapshot-hash-frozen',
      });
    } finally {
