...
// Kept from 6ef2b49a8 after its revert: this is the negative case that commit
// added to prove its viral exemption did not leak, and it stays true (and more
// load-bearing) now that `explicitConfirmationBinding` forces makeReady false
// for every merchant_confirmed plan again. §5.4 makes 开始制作 the single
// admission for paid execution, so a prepare() that claims makeReady must not
// be able to start Make behind that gate.
