...
import {
  ...
  type ProductEntitlementSupplement,
  type ProductEntitlementSupplementPort,
  ...
  implements ProductEntitlementPolicyPort, ProductEntitlementSupplementPort
{
  ...
  async activatePlan(
    ...
    const result = await this.repository.executeIdempotent(
      ...
      async (store) => {
        await authorizePlanWriter(store, context);
        let events = await store.listProductEntitlementEvents(
          context.workspaceId
        );
        // Expire any time-ended plan under the workspace lock before granting.
        events = await ensureExpiredPlansRecorded(
          store,
          context,
          events,
          this.nowMs()
        ...
        // Once-per-tenant REGISTER_GIFT: unique on (workspaceId, grantKey).
        // paymentEventId alone is insufficient — callers cannot re-key around this.
        if (input.grantKey === REGISTER_GIFT_GRANT_KEY) {
          const priorGift = events.find(
            (event) =>
        ...
        const previous = activePlanEvent(events, this.nowMs());
        ...
        const previousExpired =
          ...
          events.some(
            (candidate) =>
              candidate.kind === 'plan_expired' &&
              candidate.planEventId === latest.id,
          )
            ? latest
            : undefined;
        ...
        for (const resource of RESOURCES) {
          const amount = previous
            ...
            : previousExpired
              ? input.policy.allowance[resource] +
                Math.min(
                  previousExpired.policy.allowance[resource],
                  await periodCommittedUsage(
                    store,
                    context.workspaceId,
                    resource,
                    previousExpired.policy
                  )
                )
              : input.policy.allowance[resource] -
  ...
  async getProjection(
    context: P1Context,
    month = currentMonth(this.clock)
  ) {
    await authorizeOwner(this.repository, context);
    // Best-effort write of plan_expired under workspace lock when period ended.
    // Key is plan-event-scoped so a pre-expiry sweep does not block later expiry.
    const events = await this.repository.listProductEntitlementEvents(
      context.workspaceId
    );
    const latest = latestPlan(events);
    if (latest) {
      const endsAt = Date.parse(latest.policy.periodEndsAt);
      const alreadyExpired = events.some(
        (event) =>
          event.kind === 'plan_expired' && event.planEventId === latest.id
      );
      if (
        !alreadyExpired &&
        ...
        try {
          await this.repository.executeIdempotent(
            context,
            `plan-expire:${latest.id}`,
            digest(`plan-expire:${latest.id}`),
            async (store) => {
              await authorizeOwner(store, context);
              const locked = await store.listProductEntitlementEvents(
                context.workspaceId
              );
              await ensureExpiredPlansRecorded(
                store,
                context,
                locked,
                this.nowMs()
          ...
          // Projection remains correct via periodEndsAt zeroing even if write fails.
        }
      }
    }
    return projectionFromStore(
  ...
  async resolveSupplement(
    ...
    await this.expireEndedPlanForExecution(workspaceId);
    const events = await this.repository.listProductEntitlementEvents(
      workspaceId,
    );
    const projection = await projectionFromStore(
    ...
    return {
      ...
      planState: plan
        ? 'active'
        : events.some((event) => event.kind === 'plan_activated')
          ? 'expired'
          : 'none',
      policy: plan
        ? normalizeProductEntitlementPolicy({
            ...
            tier: plan.tier,
            // Plan events carry period bounds; surface as H1 validity window.
            validity: {
              validFrom: plan.periodStartsAt,
              validUntil: plan.periodEndsAt,
            },
          })
        : null,
  ...
  private async expireEndedPlanForExecution(workspaceId: string) {
    const events = await this.repository.listProductEntitlementEvents(
      workspaceId,
    );
    const latest = latestPlan(events);
    ...
    const endsAt = Date.parse(latest.policy.periodEndsAt);
    if (alreadyExpired || !Number.isFinite(endsAt) || endsAt > this.nowMs()) {
      return;
    }
    const context: P1Context = {
      actor: 'worker',
      correlationId: `plan-expire:${latest.id}`,
      userId: 'system-entitlement-expiry',
      workspaceId,
    };
    await this.repository.executeIdempotent(
      context,
      ...
      digest(`plan-expire:${latest.id}`),
      async (store) => {
        const locked = await store.listProductEntitlementEvents(workspaceId);
        await ensureExpiredPlansRecorded(
          store,
          context,
          locked,
...
async function projectionFromStore(
  ...
  const active = activePlanEvent(events, now);
  // When the latest plan has ended, surface zero allowance (fix periodEndsAt bug).
  const plan = active?.policy ?? null;
  const expiredLatest = !active ? latestPlan(events) : null;
  const expirationRecorded = expiredLatest
    ? events.some(
        (event) =>
          event.kind === 'plan_expired' &&
          event.planEventId === expiredLatest.id,
      )
    : false;
  ...
  const usage = Object.fromEntries(
    RESOURCES.map((resource, index) => {
      const projected = projectUsage(usageLists[index] as UsageEvent[]);
      if (!expiredLatest || expirationRecorded) return [resource, projected];
      // Read fallback when recording plan_expired failed: zero only the plan
      // opening while retaining separately purchased add-on quantity.
      const planPortion = expiredLatest.policy.allowance[resource];
      const adjustedAllowance = Math.max(
        0,
        projected.allowance - planPortion
      );
      const consumed =
        projected.reserved + projected.committed - projected.released;
      return [
        resource,
        {
          ...projected,
          allowance: adjustedAllowance,
          available: Math.max(0, adjustedAllowance - consumed),
        },
      ];
    })
...
async function authorizePlanWriter(
  store: FoundationStore,
  context: P1Context,
) {
  if (context.actor === 'payment') return;
  await authorizeOwner(store, context);
}
...
/** Latest plan only, when still within periodEndsAt and not explicitly expired. */
function activePlanEvent(
  events: ProductEntitlementEvent[],
  now = Date.now()
): Extract<ProductEntitlementEvent, { kind: 'plan_activated' }> | undefined {
  const plan = latestPlan(events);
  if (!plan) return undefined;
  const expiredAfter = events.some(
    (event) => event.kind === 'plan_expired' && event.planEventId === plan.id,
  );
  if (expiredAfter) return undefined;
  const endsAt = Date.parse(plan.policy.periodEndsAt);
  return Number.isFinite(endsAt) && endsAt <= now ? undefined : plan;
}

/**
 * When the active plan's period has ended, append plan_expired and reverse the
 * remaining plan opening allowance. Called under the workspace lock.
 */
async function ensureExpiredPlansRecorded(
  store: FoundationStore,
  context: P1Context,
  events: ProductEntitlementEvent[],
  now = Date.now()
): Promise<ProductEntitlementEvent[]> {
  const latest = latestPlan(events);
  ...
  if (alreadyExpired) return events;
  const endsAt = Date.parse(latest.policy.periodEndsAt);
  if (!Number.isFinite(endsAt) || endsAt > now) return events;

  const expireEvent: ProductEntitlementEvent = {
    id: `plan-expired-${digest(latest.id).slice(0, 24)}`,
    workspaceId: context.workspaceId,
    actorId: context.userId,
    correlationId: context.correlationId,
    createdAt: new Date(now).toISOString(),
    kind: 'plan_expired',
    planEventId: latest.id,
    policy: structuredClone(latest.policy),
    reason: 'period_ended',
  };
  await store.appendProductEntitlementEvent(expireEvent);

  for (const resource of RESOURCES) {
    // The usage projection is current entitlement capacity, not lifetime
    // allocation. Completed usage remains auditable in commit events.
    const amount = -latest.policy.allowance[resource];
    if (amount !== 0) {
      await appendAllowanceAdjustment(
        store,
        expireEvent,
        resource,
        amount,
        `plan_expired:${latest.policy.tier}:${latest.policy.revision}:${latest.policy.periodId}`
      );
    }
  }

  return store.listProductEntitlementEvents(context.workspaceId);
}
...
function validatePlan(policy: ProductPlanPolicy) {
  ...
  if (policy.tier === 'trial' && policy.periodStrategy !== 'fixed_days') {
    throw new P1DomainError(
      'INVALID_STATE',
      'Trial plans require the fixed_days period strategy.'
    );
  }
  if (policy.tier !== 'trial' && policy.periodStrategy === 'fixed_days') {
    throw new P1DomainError(
      'INVALID_STATE',
      'Only trial plans may use the fixed_days period strategy.'
    );
  }
  const startsAt = Date.parse(policy.periodStartsAt);
  const endsAt = Date.parse(policy.periodEndsAt);
...
function currentMonth(clock: () => Date = () => new Date()) {
