...
export type MediaAdmissionCrashMode = 'wait' | 'after-claim' | 'after-terminal';
  ...
  const snapshot = createCreationExecutionSnapshot(
    {
      ...
      route: { id: 'route-s6-admission', revision: 'model-r1' },
        ...
        allowedCandidates: [
          {
            ...
            modelDisplayName: 'S6 media admission fixture',
            ...
            stableModelName: 's6-media-admission',
            modelVersion: '1',
            credentialMode: 'platform',
            credentialVersion: 'credential-r1',
            policyRevision: snapshot.modelPolicy.revision,
...
function crashAdmission(
  ...
  return {
    async claim(input: Parameters<NoteMediaAdmissionPort['claim']>[0]) {
      ...
      if (!token) nullClaims += 1;
      const shouldCrash =
        (mode === 'wait' && !token && nullClaims >= 2) ||
        (mode === 'after-claim' && token);
    ...
    async markTerminal(token: NoteMediaAdmissionToken, status: 'completed' | 'failed') {
      const result = await noteAdmission.markTerminal(token, status);
      if (mode === 'after-terminal' && result && !signalled) {
        signalled = true;
        process.stdout.write('ADMISSION_TERMINAL_WRITTEN\n', () => {
          process.kill(process.pid, 'SIGKILL');
        });
        await new Promise<void>(() => {});
      }
...
function mediaAdmissionContext(
  ...
  return {
    ...
    policyReferences: {
      ...
      identityRefs: [
        {
          id: 'marketing_identity:identity-s6-admission:identity-r1',
          workspaceId,
          status: 'registered',
        },
      ],
