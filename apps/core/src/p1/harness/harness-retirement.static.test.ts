...
test('core production source cannot resurrect the retired copy bypass or scorer', () => {
  const files = execFileSync('rg', ['--files', 'apps/core/src'], {
    cwd: repositoryRoot,
    encoding: 'utf8',
  })
    .trim()
...
test('copy production no longer calls scene-policy projection and capabilities are derived', () => {
  ...
  const legacyCallers = execFileSync(
    'rg',
    ['--files', 'apps/core/src/p1/harness'],
  ...
  assert.deepEqual(deriveMarketingPackageCapabilities(), {
