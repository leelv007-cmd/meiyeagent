...
test("NotePlan model runners consume the three frozen prompt contents", async () => {
	...
	assert.deepEqual(
		...
		["frozen:note-plan", "frozen:note-text-block", "frozen:note-consistency"],
	);
});
...
test("viral rewrite materializes merchant and fact context into the canonical NotePlan schema", async () => {
	...
	const port = new ModelSupplyNotePlanStructuredPort(
		runner,
		"workflow-viral-rewrite",
		() => "2026-08-02T00:00:00.000Z",
		undefined,
		{
			notePlan: frozenPrompt(
				"harness/note-plan",
				"BASE NOTE PLAN note-plan/v1",
			),
			xhsViralRewrite: frozenPrompt(
				"harness/xhs-viral-rewrite",
				[
					"参考笔记原文：",
					"{sourceNote}",
					"门店/项目语境：",
	...
	assert.ok(request);
	assert.equal(request.schemaName, "harness_note_plan_v1");
	assert.equal(request.schemaRevision, "note-plan-v1");
	assert.match(request.instructions, /清爽护理三步走/u);
	assert.match(request.instructions, /项目名：夏日清爽护理/u);
	assert.match(request.instructions, /note-plan\/v1/u);
	assert.doesNotMatch(request.instructions, /\{sourceNote\}|\{shopContext\}/u);
	assert.doesNotMatch(request.instructions, /"sourceTrack"\s*:\s*"paste"/u);
});
