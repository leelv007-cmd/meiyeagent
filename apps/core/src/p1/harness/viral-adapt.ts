...
export const viralImageVisionResultSchema = z
	.object({
		styleSummary: z.string().trim().min(1),
		englishImagePrompt: z.string().trim().min(1),
		onImageChineseText: z.array(z.string().trim().min(1)),
		negativePrompt: z.string().trim().min(1),
		sourceTrack: z.literal("paste"),
	})
	.strict();

export type ViralImageVisionResult = z.infer<
	typeof viralImageVisionResultSchema
>;
 ...
 */
export function resolveOpenCliLiveGate(
...
export function availableViralSourcingTracks(
	...
	return gate.available
		? (["paste", "opencli_link"] as const)
		: (["paste"] as const);
}

export function isOpenCliTrackSelectable(gate: ViralOpenCliLiveGate): boolean {
	return gate.available;
}
 ...
 */
export function fixtureViralRewrite(
	source: ViralPasteSource,
): ViralRewriteResult {
...
export function runViralAdaptPasteToNoteProjection(input: {
	...
	| {
			ok: true;
			confirm: ViralAdaptConfirmProjection;
			package: ViralAdaptNotePackageProjection;
			rewrite: ViralRewriteResult;
	  }
...
export function notePlanInstructionsForViralAdapt(input: {
	...
	if (/\{sourceNote\}|\{shopContext\}/u.test(viral)) {
		throw new Error("Viral rewrite prompt placeholders were not materialized.");
	}
	return {
		...
		instructions: [
			viral,
			"---",
			"唯一输出契约：note-plan/v1。不得输出并行的 title/body/tags/sourceTrack 根对象。",
...
export function materializeViralImageVisionPrompt(input: {
	...
	const materialized = template
		.replaceAll(
			"{imageNotes}",
			() => `已授权的 Composer 参考资产：${input.assetIds.join(", ")}`,
		)
		.replaceAll("{shopContext}", () => input.shopContext);
	if (/\{imageNotes\}|\{shopContext\}/u.test(materialized)) {
		throw new Error(
			"Viral image-vision prompt placeholders were not materialized.",
		);
	}
	return materialized;
}

/** Keep the VLM result in the canonical image intent that feeds page generation. */
export function applyViralImageVisionToNotePlan(
	plan: NotePlan,
	vision: ViralImageVisionResult,
): NotePlan {
	return {
		...plan,
		pages: plan.pages.map((page) => ({
			...
			imageIntent: {
				...page.imageIntent,
				scene: `${page.imageIntent.scene}；参考图风格：${vision.styleSummary}`,
				composition: [
					page.imageIntent.composition,
					vision.englishImagePrompt,
					`图上中文建议：${vision.onImageChineseText.join(" / ")}`,
					`Negative prompt: ${vision.negativePrompt}`,
				].join("；"),
			},
		})),
	};
