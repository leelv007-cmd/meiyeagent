...
import { strictByokLedgerRouteSnapshots } from '../route-snapshot-normalize.js';
import type {
  IntegrationContext,
...
export class FoundationStrictByokLedger implements StrictByokLedgerPort {
  ...
  async prepare(input: Parameters<StrictByokLedgerPort['prepare']>[0]) {
    ...
    // S2b: public + foundation checkpoint built from the same canonical snapshot.
    const { publicSnapshot: routeSnapshot, foundationCheckpoint } =
    ...
    const checkpoint = await this.foundation.checkpointGenerationAttempt(
      context,
      {
        ...
        routeSnapshot: foundationCheckpoint,
        attempt: {
          id: attemptId,
          deploymentId: foundationCheckpoint.allowedCandidates[0]!.deploymentId,
        },
      },
