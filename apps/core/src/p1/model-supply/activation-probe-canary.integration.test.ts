...
describe('Ticket 21 activation canary', () => {
  ...
  it('persists classified failures without replacing the last successful evidence', async () => {
    ...
    const history = await harness.repository.listActivationProbeRuns(
      admin.workspaceId
    );
    ...
    const status = await harness.controlPlane.activationStatus(
      admin.workspaceId
    );
    assert.equal(
      status.find((candidate) => candidate.deploymentId === deploymentId)
        ?.latestProbe?.id,
      failed.id
    );
    assert.equal(
      status.find((candidate) => candidate.deploymentId === deploymentId)
        ?.latestProbe?.failureCategory,
      'submit:quota_exhausted'
    );
  });
});
