...
export function fileSystemAssetStorageFromEnv(env: NodeJS.ProcessEnv) {
  return new FileSystemAssetStorage({
    rootDirectory: env.P1_ASSET_STORAGE_DIR ?? './.data/p1-assets',
    publicBaseUrl:
  ...
  implements ModelAssetStoragePort, CompositionAssetStoragePort
{
  ...
  private async persistBytes<T extends CustodyOwnedAssetContentType>(input: {
    ...
    const receiptDigest = generatedReceiptDigest(
      sha256,
      input.sourceTaskRef,
    );
...
async function probeVideo(path: string, ffprobePath: string): Promise<VideoProbeResult> {
...
function assertPublicObjectKey(objectKey: string) {
  if (
    !/^[A-Za-z0-9._-]+\/(?:(?:generated|owned)\/[a-f0-9]{64}\.(?:jpg|png|webp|mp4|m4a|zip|mp3|ogg|wav)|composed\/[a-f0-9]{64}\.(?:png|mp4))$/.test(
      objectKey
    )
  ) {
...
function contentTypeForExtension(extension: string): CustodyOwnedAssetContentType {
  switch (extension) {
    case '.jpg':
    case '.jpeg':
      return 'image/jpeg';
