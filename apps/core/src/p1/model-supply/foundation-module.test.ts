...
import { isAudioProductionGenerationAllowed } from './audio-activation-gate.js';
...
describe('ModelSupplyFoundationModule', () => {
  ...
  it('marks recorded deployments executable only for the explicit local fixture runtime', async () => {
    ...
    const speechDeployment = fixture.deployments.find(
      (deployment) => deployment.catalogModelId === 'audio-speech-fixture',
    );
    // Production gate keeps fixture audio closed — no real provider/price/probe.
    assert.equal(
