  ...
  implements ModelSupplyControlPlaneRepository
{
  ...
  async listActivationProbeRuns(workspaceId: string) {
    return structuredClone(this.activationProbeRuns.get(workspaceId) ?? []);
  }
...
export class ModelSupplyControlPlaneService {
  ...
  async getCanvasGenerationCatalog(workspaceId: string) {
    ...
    return {
      ...
      operations: operations.map((operation) => {
        ...
        return {
          activation: active.length > 0 ? ('active' as const) : ('inactive' as const),
          allowedInputAssetRoles: [
            ...new Set(active.flatMap(({ capability }) => capability.inputAssetRoles)),
          ],
          allowedParameters: [
            ...new Set(active.flatMap(({ capability }) => capability.parameters)),
          ],
          estimatedDurationSeconds: [5, 60] as [number, number],
          modelId: active[0]?.deployment.catalogModelId ?? null,
          operation,
          output: operation === 'text.respond'
            ...
            : operation.startsWith('image.')
              ...
              : operation === 'video.generate'
                ? ('video' as const)
                : ('audio' as const),
          usageAmount: canvasProductUsageQuantity(
            active[0]?.deployment,
            this.allowRecordedExecution,
          ),
  ...
  async getAdminCatalogControl(workspaceId: string) {
    ...
    return {
      ...
      revisionId: source.revisionId,
      stage: source.stage,
      catalog: adminCatalogPayload(source.payload),
    };
  }
...
function videoWorkflowShots(value: unknown): VideoWorkflowShotInput[] {
  if (!Array.isArray(value) || value.length === 0) {
...
const CANVAS_GENERATION_REQUEST_KEYS = new Set([
  'dataClass', 'inputAssets', 'modelId', 'operation',
      ...
      case 'video_workflow_create_draft':
        return this.requireComposedVideo().createDraft({
          ...
          payload.brandWatermarkText.trim()
            ...
            : {}),
          shots: videoWorkflowShots(payload.shots),
        });
