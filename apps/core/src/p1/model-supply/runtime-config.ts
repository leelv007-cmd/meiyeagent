...
export function modelMediaExecutionMode(
  runtime: Pick<
    ...
    'arkMedia' | 'media' | 'mode' | 'tuziMedia'
  >,
) {
  if (runtime.mode === 'fixture' && runtime.media) return 'fixture' as const;
  if (runtime.arkMedia && runtime.tuziMedia) return 'ark,tuzi' as const;
  if (runtime.arkMedia) return 'ark' as const;
  if (runtime.tuziMedia) return 'tuzi' as const;
...
export function modelRuntimeAssemblyFromEnv(
  ...
  const arkMedia = mediaMode.has('ark') ? arkMediaOptions(env) : undefined;
  const tuziMedia = mediaMode.has('tuzi') ? tuziMediaOptions(env) : undefined;
  ...
  const directDeploymentId = direct
    ...
    : undefined;
  const directEvidence = directConfigurationRevision
  ...
  const tuziImageEvidence = tuziRevisions
    ? verifiedProbeEvidence(
        activationEvidenceByDeploymentId['gpt-image-2-tuzi-relay'],
        tuziRevisions.image
      )
    : undefined;
  const tuziVideoEvidence = tuziRevisions
    ? verifiedProbeEvidence(
        activationEvidenceByDeploymentId['seedance-2-tuzi-relay'],
        tuziRevisions.video
      )
    : undefined;
  const gateway =
  ...
  if (mode === 'fixture') {
    for (const deployment of documented) activeDeploymentIds.add(deployment.id);
  } else {
    ...
    if (tuziImageEvidence) {
      activeDeploymentIds.add('gpt-image-2-tuzi-relay');
    }
    if (tuziVideoEvidence) {
      activeDeploymentIds.add('seedance-2-tuzi-relay');
    }
  }
  ...
  if (tuziImageEvidence) {
    explicitActivationEvidence['gpt-image-2-tuzi-relay'] = tuziImageEvidence;
  }
  if (tuziVideoEvidence) {
    explicitActivationEvidence['seedance-2-tuzi-relay'] = tuziVideoEvidence;
  }
  const deployments = createDefaultDeployments({
    ...
    if (
      tuziMedia &&
      (deployment.id === 'gpt-image-2-tuzi-relay' ||
  ...
  if (tuziRevisions) {
    configurationRevisions['gpt-image-2-tuzi-relay'] = tuziRevisions.image;
    configurationRevisions['seedance-2-tuzi-relay'] = tuziRevisions.video;
  }
  return {
...
function mediaConfigurationRevisions(options: ArkMediaExecutionOptions) {
  const common = {
...
function parseMediaMode(env: NodeJS.ProcessEnv): ReadonlySet<'ark' | 'tuzi'> {
  ...
  if (
    ...
    providers.some((provider) => provider !== 'ark' && provider !== 'tuzi')
  ) {
    throw new Error(
      'MODEL_MEDIA_EXECUTION_MODE must be disabled, ark, tuzi, or ark,tuzi.'
    );
  }
  return new Set(providers as Array<'ark' | 'tuzi'>);
}
