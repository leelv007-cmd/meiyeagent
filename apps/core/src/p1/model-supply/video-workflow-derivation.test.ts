...
function frozenVideoContract(
  ...
  return {
    ...
    currency: 'CNY',
    dataClass: [],
    // Recorded candidates are 15s each (in-memory storage), so a two-shot
    // composition runs 30s; the frozen contract must match the clips-first
    // duration (V31-61) or provenance validation fails.
    durationSeconds: 30,
    estimatedAmount: 12,
    operation: 'video.generate',
    outputCount: 1,
    outputLabel: '30 second composed video',
