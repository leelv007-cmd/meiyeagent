...
export class VolcengineBidirectionalTtsAdapter {
  ...
  async synthesize(
    ...
    try {
      ...
      const acceptAudio = (frame: VolcengineTtsFrame) => {
        if (frame.event !== VOLCENGINE_TTS_EVENT.TTSResponse) return false;
        if (
      ...
      while (!synthesisStarted) {
        ...
        if (acceptAudio(frame)) continue;
        if (frame.event === VOLCENGINE_TTS_EVENT.TTSSentenceStart) {
      ...
      while (true) {
        ...
        throw new VolcengineTtsAdapterError(
          'invalid_response',
          `Volcengine TTS returned unexpected post-synthesis event ${String(frame.event)}.`,
        );
      }
