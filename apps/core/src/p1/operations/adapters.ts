...
export class RecordedBatchExecutionAdapter implements BatchExecutionPort {
  async execute(request: Parameters<BatchExecutionPort['execute']>[0]) {
    const artifactKind =
      request.action === 'apply_template'
        ? ('work' as const)
        : request.action === 'prepare_draft'
          ? ('draft' as const)
          : ('content' as const);
    return {
      output: {
        artifactId: `recorded-${artifactKind}-${request.executionId}`,
        artifactKind,
        metadata: { adapter: 'recorded', action: request.action },
      },
...
class LocalBatchExecutionError extends Error {
...
/**
 * Executes weekly actions against the existing Product and Operations facts.
 * Product commands already persist idempotency receipts; template work uses the
 * Operations command receipt with the same stable execution id. This makes a
 * recovered weekly lease return the original artifact instead of creating a
 * second content item or canvas work.
 */
export class ProductOperationsBatchExecutionAdapter
  implements BatchExecutionPort
{
  constructor(
    private readonly product: ProductApplicationService,
  ...
  async execute(request: BatchExecutionRequest): Promise<BatchExecutionResult> {
    try {
      if (request.action === 'apply_template') {
        return await this.applyTemplate(request);
      }
      return await this.executeContentAction(request);
    } catch (cause) {
      ...
      return {
        ...
        errorMessage:
          typeof error.message === 'string'
            ? error.message
            : 'The weekly action could not be executed.',
  ...
  private productContext(request: BatchExecutionRequest) {
    return {
      actor: 'user' as const,
  ...
  private operationContext(request: BatchExecutionRequest) {
    return {
  ...
  private async sourceContentId(request: BatchExecutionRequest) {
    const context = this.productContext(request);
    const state = await this.product.bootstrap(context);
    const requestedId =
      request.task.relatedObject?.kind === 'content'
        ? request.task.relatedObject.id
        : undefined;
    const source = requestedId
      ? state.contents.find((content) => content.id === requestedId)
      : [...state.contents]
          .filter((content) => content.status !== 'abandoned')
          .sort(
            (left, right) =>
    ...
    if (!source) {
      throw new LocalBatchExecutionError(
        'SOURCE_CONTENT_REQUIRED',
        'Add or select a content draft before running this weekly action.'
      );
    }
    return source.id;
  }
  ...
  private async executeContentAction(
    ...
    const sourceContentId = await this.sourceContentId(request);
    const command =
      request.action === 'revise'
        ? {
            contentId: sourceContentId,
            instruction: 'conversational' as const,
            type: 'quick_edit' as const,
          }
        : { contentId: sourceContentId, type: 'remix_content' as const };
    const result = await this.product.execute(
      this.productContext(request),
      command,
      `weekly-batch-effect:${request.executionId}`
    );
    const contentId = result.output.contentId;
    if (
      !contentId ||
      !result.state.contents.some((content) => content.id === contentId)
    ) {
      throw new LocalBatchExecutionError(
        'CONTENT_EFFECT_MISSING',
        'The Product command completed without a queryable content fact.',
        500
      );
    }
    return {
      output: {
        ...
        artifactKind: request.action === 'prepare_draft' ? 'draft' : 'content',
        metadata: {
  ...
  private async applyTemplate(
    ...
    const context = this.operationContext(request);
    return operations.executeIdempotentModuleCommand(
      context,
      `weekly-batch-effect:${request.executionId}`,
      {
        action: request.action,
      ...
      async () => {
        const templates = await operations.listTemplates(context, {
          publicationStatuses: ['enabled', 'published'],
        });
        const requestedTemplateId =
          request.task.relatedObject?.kind === 'template'
            ? request.task.relatedObject.id
            : undefined;
        const template = requestedTemplateId
          ? templates.find((candidate) => candidate.id === requestedTemplateId)
          : templates[0];
        if (!template) {
          throw new LocalBatchExecutionError(
            'TEMPLATE_REQUIRED',
            'Choose an available official template before applying this weekly action.'
          );
        }
        const work = await operations.createWork(context, {
          name: request.task.title,
          templateId: template.id,
        });
        return {
          output: {
            artifactId: work.id,
            artifactKind: 'work' as const,
            metadata: {
              action: request.action,
              entityKind: 'work',
              templateId: template.id,
              templateVersionId: work.templateVersionId ?? 'unknown',
            },
          },
          status: 'completed' as const,
        };
