...
describe('ticket 13 advanced canvas package library lifecycle', () => {
  it('exports and reuses an advanced-canvas adopted package like any other ContentPackage', async () => {
    let exportEffects = 0;
    ...
    const exported = (await service.executeModule(
      ...
      'operations',
      {
        ...
        payload: { packageId: source.id, platform: 'xiaohongshu' },
      },
    ...
    assert.equal(reused.versions[0]?.body, '画布采用正文：今日猫眼美甲');
  });
});
