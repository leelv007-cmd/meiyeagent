...
describe('P1 operations application service', () => {
  ...
  it('rejects malformed or forged promotional material receipts before persistence', async () => {
    const { service } = setup();
    ...
    await assert.rejects(
      service.exportWork(owner, work.id, {
        ...
        promotionalMaterialReceipt: {
          ...
          missingMaterialFallback: 'none',
          outputSha256: 'not-a-sha256',
          provenanceRef: revision.id,
        },
      } as never),
      /Invalid promotional material receipt/u
    );
    await assert.rejects(
      service.exportWork(owner, work.id, {
        ...request,
        promotionalMaterialReceipt: {
          ...
          missingMaterialFallback: 'none',
          outputSha256,
          provenanceRef: 'forged-client-revision',
        },
      } as never),
      /provenanceRef must match workRevisionId/u
    );
    assert.deepEqual(await service.listExportReceipts(owner, work.id), []);
  });

  it('persists and reads back a validated promotional material receipt intact', async () => {
    const { service } = setup();
    const spec = PROMOTIONAL_MATERIAL_SPECS[2];
    const work = await service.createBlankWork(owner, {
      height: spec.height,
    ...
    const promotionalMaterialReceipt = {
      ...
      missingMaterialFallback: 'brand_safe_placeholder' as const,
      outputSha256,
      provenanceRef: revision.id,
    };
    ...
    assert.deepEqual(
      (await service.listExportReceipts(owner, work.id))[0]
        ?.promotionalMaterialReceipt,
      promotionalMaterialReceipt
    );
  });
