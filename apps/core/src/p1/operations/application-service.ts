...
export class OperationsApplicationService {
  ...
  private async mutate<T>(
    context: OperationContext,
    action: (state: OperationsWorkspaceState) => Promise<T> | T
  ) {
    await this.authorize(context);
    return this.repository.withWorkspaceLock(
      context.workspaceId,
  ...
  async startCreativeCopyStream(
    ...
    const started = await executor.startCopyStream({
      ...
      groundingSnapshot: structuredClone(prepared.groundingSnapshot),
      idempotencyKey: submissionKey,
    ...
    return {
      ...
      completion: started.completion.then((outcome) =>
        this.applyCreativeOutcome(context, prepared.jobId, outcome)
      ),
    };
  }
  ...
  async reconcileVideoContentPackage(
    ...
    return this.mutate(context, async (state) => {
      ...
      if (input.status === 'awaiting_quality_review') {
        ...
        const ownedPrefix = `${context.workspaceId}/`;
        if (
          input.composedAsset.contentType !== 'video/mp4' ||
          !input.composedAsset.objectKey.startsWith(ownedPrefix) ||
          input.composedAsset.objectKey.includes('://')
        ) {
          throw new OperationsError(
            'VIDEO_CONTENT_PACKAGE_ASSET_NOT_OWNED',
            'Completed video must reference an owned object key.',
            409
          );
        }
  ...
  async repairMediaCustody(
    ...
    return this.mutate(context, async (state) => {
      ...
      try {
        ...
        state.contentPackages = repaired.contentPackages as ContentPackage[];
        this.audit(
          state,
          context,
  ...
  async editContentPackageVersion(
    ...
    return this.mutate(context, async (state) => {
      ...
      if (
        !['accepted', 'export_failed', 'review_ready'].includes(current.status)
      ) {
        throw new OperationsError(
          'CONTENT_PACKAGE_TRANSITION_CONFLICT',
      ...
      const ownedAssetIds = new Set([
        ...
        ...current.versions.flatMap((version) => version.orderedAssetIds),
      ]);
      ...
      const version = {
        ...
        id: versionId,
        source: 'merchant_edited' as const,
      };
      const updated = {
        ...current,
      ...
      this.audit(
        ...
        'content_package',
        current.id,
        { baseVersionId: input.baseVersionId, versionId }
      );
      return {
        ...updated,
        ...contentPackageVisibleStatus(updated.status),
      };
    });
  }
  ...
  async generateContentPackageVariants(
    ...
    if (contentPackage.status !== 'accepted') {
      throw new OperationsError(
        ...
        'Only an accepted ContentPackage can generate platform variants.',
        409
      );
    }
      ...
      await this.dependencies.contentPackageRightsResolver?.resolve({
        assetIds: [
          ...new Set([
            ...
            ...currentVersion.orderedAssetIds,
          ]),
        ],
        workspaceId: context.workspaceId,
      });
    ...
    return this.mutate(context, async (current) => {
      ...
      current.contentPackages[index] = updated;
      this.audit(
        current,
        context,
  ...
  async editContentPackageVariant(
    ...
    return this.mutate(context, async (state) => {
      ...
      if (variant.currentVersionId !== input.baseVersionId) {
        throw new OperationsError(
          'CONTENT_PACKAGE_VERSION_CONFLICT',
      ...
      const ownedAssetIds = new Set([
        ...
        ...current.generated.assetIds,
        ...current.versions.flatMap((version) => version.orderedAssetIds),
        ...current.variants.flatMap((item) =>
          item.versions.flatMap((version) => version.orderedAssetIds)
        ),
      ]);
      if (input.changes.orderedAssetIds.some((id) => !ownedAssetIds.has(id))) {
        throw new OperationsError(
      ...
      const timestamp = this.timestamp();
      const versionId = `${variant.id}-v-${createHash('sha256')
        .update(
          JSON.stringify({
            baseVersionId: input.baseVersionId,
            changes: input.changes,
      ...
      const updatedVariant = {
        ...variant,
        currentVersionId: versionId,
        versions: [...variant.versions, version],
      };
      const updated = {
        ...current,
        updatedAt: timestamp,
  ...
  async rollbackContentPackageVersion(
    context: OperationContext,
    input: { packageId: string; targetVersionId: string }
  ) {
    return this.mutate(context, async (state) => {
      await this.requireContentPackageWrite(context);
      state.contentPackages ??= [];
      ...
      const targetPackageVersion = current.versions.find(
        (version) => version.id === input.targetVersionId
      );
      const variantIndex = current.variants.findIndex((variant) =>
      ...
      if (
        current.currentVersionId === input.targetVersionId ||
      ...
      const timestamp = this.timestamp();
      const ownerId = variant?.id ?? current.id;
      const versionId = `${ownerId}-rollback-${createHash('sha256')
        .update(
          `${input.targetVersionId}:${variant?.currentVersionId ?? current.currentVersionId}`
        )
        .digest('hex')
        .slice(0, 16)}`;
      const restored = {
        ...
        source: 'rollback_restored' as const,
        title: target.title,
      ...
      const updated = variant
            ...
            variants: current.variants.map((item, index) =>
              index === variantIndex
                ? {
                    ...item,
                    currentVersionId: versionId,
                    versions: [...item.versions, restored],
                  }
                : item
            ),
          }
        : {
            ...
            currentVersionId: versionId,
            updatedAt: timestamp,
            versions: [...current.versions, restored],
          };
      state.contentPackages[packageIndex] = updated;
      this.audit(
        state,
  ...
  async reuseContentPackage(
    context: OperationContext,
    input: { sourcePackageId: string; versionId?: string }
  ) {
    return this.mutate(context, async (state) => {
      await this.requireContentPackageWrite(context);
      state.contentPackages ??= [];
      const source = state.contentPackages.find(
        (candidate) => candidate.id === input.sourcePackageId
      );
      if (!source) {
        throw new OperationsError(
          'CONTENT_PACKAGE_NOT_FOUND',
          'The source ContentPackage was not found.',
          404
        );
      }
      if (
        !['accepted', 'review_ready'].includes(source.status) ||
        source.rights.state === 'revoked'
      ) {
        throw new OperationsError(
          'REUSE_SOURCE_REVOKED',
          'Only a usable ContentPackage with active rights can be reused.',
          409
        );
      }
      const selectedVersionId = input.versionId ?? source.currentVersionId;
      const selectedVersion = source.versions.find(
      ...
      // 与导出同一口径的 live rights 复核：素材撤权即使尚未传播到包状态，
      // 也不得被「做同款」复制进新成品（撤权后阻断所有路径的不变量）。
      const liveRights =
        await this.dependencies.contentPackageRightsResolver?.resolve({
          assetIds: [
            ...new Set([
              ...source.source.assetIds,
              ...selectedVersion.orderedAssetIds,
            ]),
          ],
          workspaceId: context.workspaceId,
        });
      if (liveRights && liveRights.unauthorizedAssetIds.length > 0) {
        throw new OperationsError(
          'REUSE_SOURCE_REVOKED',
          'Live Product rights block reusing this ContentPackage.',
          409
        );
      }
      const timestamp = this.timestamp();
      ...
      const reused = transitionContentPackage(
        {
          ...draft,
        ...
        },
        {
          ...
          version: {
            ...
            ...(selectedVersion.conversionHook
              ? { conversionHook: selectedVersion.conversionHook }
              : {}),
            createdAt: timestamp,
            createdBy: context.userId,
            derivedFromVersionId: selectedVersion.id,
            id: versionId,
            orderedAssetIds: [...selectedVersion.orderedAssetIds],
      ...
      state.contentPackages.push(reused);
      this.audit(
        state,
        context,
        'content_package.reused',
        'content_package',
        reused.id,
        { sourcePackageId: source.id, sourceVersionId: selectedVersion.id }
      );
      this.creationEvent(state, context, 'first_content_package_created', {
        contentPackageId: reused.id,
      });
      return {
        ...reused,
        ...contentPackageVisibleStatus(reused.status),
      };
    });
  }
  ...
  async cancelContentPackage(context: OperationContext, packageId: string) {
    return this.mutate(context, async (state) => {
      await this.requireContentPackageWrite(context);
      state.contentPackages ??= [];
      const index = state.contentPackages.findIndex(
    ...
    return this.mutate(context, (state) => {
      ...
      state.contentPackages = state.contentPackages.map((contentPackage) => {
        const referencesAsset =
          contentPackage.source.assetIds.includes(assetId) ||
          contentPackage.generated.assetIds.includes(assetId) ||
          contentPackage.generated.ownedAssets?.some(
            (asset) => asset.id === assetId
          ) ||
          contentPackage.versions.some((version) =>
            version.orderedAssetIds.includes(assetId)
          ) ||
          contentPackage.variants.some((variant) =>
            variant.versions.some((version) =>
              version.orderedAssetIds.includes(assetId)
            )
          );
        if (!referencesAsset || contentPackage.rights.state === 'revoked') {
        ...
        try {
          ...
          revokedPackageIds.push(revoked.id);
          this.audit(
            state,
            context,
            'content_package.rights_revoked',
