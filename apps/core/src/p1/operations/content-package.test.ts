...
async function seedAcceptedPackage(
  ...
  const created = await operationsService.createContentPackage(
    { ...context, actor: 'owner' },
    { kind, source: { assetIds: [`source-${kind}`] } }
  );
...
describe('ContentPackage application service contract', () => {
  ...
  it('reuses one accepted version as a new package with navigable lineage', async () => {
    ...
    const reused = (await service.executeModule(
      context,
      'operations',
      {
    ...
    const lineage = (await service.queryModule(context, 'operations', {
      action: 'content_package_lineage',
      payload: { packageId: reused.id },
    })) as {
      ancestors: Array<{ id: string }>;
      children: Array<{ id: string }>;
    };
    const sourceLineage = (await service.queryModule(context, 'operations', {
      action: 'content_package_lineage',
      payload: { packageId: source.id },
    })) as typeof lineage;

    assert.deepEqual(replayed, reused);
    assert.notEqual(reused.id, source.id);
    assert.equal(reused.lineage.reusedFromPackageId, source.id);
    assert.equal(reused.versions[0]?.body, source.versions[0]?.body);
    assert.deepEqual(reused.variants, []);
    assert.deepEqual(reused.exportReceipts, []);
    assert.deepEqual(
      lineage.ancestors.map((item) => item.id),
      [source.id]
    );
    assert.deepEqual(
  ...
  it('exports the selected current variant once and persists a verifiable receipt', async () => {
    ...
    const command = {
      ...
      payload: { packageId: stored.id, platform: 'xiaohongshu' },
    };
  ...
  it('records every failed export retry without throwing a transition error', async () => {
    ...
    const first = await operationsService.exportContentPackage(
      { ...context, actor: 'owner' },
      { packageId: stored.id, platform: 'xiaohongshu' }
    );
  ...
  it('blocks reuse when live Product rights revoke a source asset before propagation lands', async () => {
    const { context, operations, operationsService } = setup({
      contentPackageRightsResolver: {
        async resolve(input) {
          assert.deepEqual(input.assetIds.sort(), [
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof OperationsError &&
        error.code === 'REUSE_SOURCE_REVOKED'
    );
    const reloaded = await operations.loadWorkspace(context.workspaceId);
    assert.equal(reloaded?.contentPackages.length, 1);
  });
  ...
  it('keeps platform version histories isolated and restores a target as a new version', async () => {
    ...
    await service.executeModule(
      ...
      'operations',
      {
        ...
        payload: { packageId: stored.id, targetVersionId: 'douyin-v1' },
      },
      'rollback-douyin-v1'
    );
  ...
  it('generates all three platform variants once through the existing application seam for either package kind', async () => {
    let providerEffects = 0;
    ...
    for (const kind of ['image_text', 'video'] as const) {
      ...
      const variantRun = (
        ...
        .find((item) => item.id === contentPackage.id)
        ?.generated.childRuns.find((run) => run.runType === 'model_job');
  ...
  it('revokes package rights and immediately projects the package as needing attention', async () => {
    ...
    const revoked = await service.executeModule<
      { action: string; payload: { packageId: string; reason: string } },
      {
...
describe('ContentPackage frozen status contract', () => {
  ...
  it('keeps historical direct-MP4 receipts readable while requiring ZIP for new exports', () => {
    const version = {
      body: '历史视频正文',
      createdAt: NOW,
    ...
    assert.throws(
      ...
      (error: unknown) =>
        error instanceof ContentPackageTransitionError &&
        /ZIP receipt/u.test(error.message)
    );
  });
