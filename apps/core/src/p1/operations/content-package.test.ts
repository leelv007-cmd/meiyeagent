...
async function seedAcceptedPackage(
  ...
  let state = await operations.loadWorkspace(context.workspaceId);
  if (!state) {
    await operationsService.createTask(
      { ...context, actor: 'owner' },
      {
        dueAt: NOW,
        executable: false,
        relatedObject: { id: 'test-seed', kind: 'content' },
        risk: 'normal',
        source: 'stale_draft',
        title: 'test workspace seed',
      }
    );
    state = await operations.loadWorkspace(context.workspaceId);
    assert.ok(state);
    state.tasks = [];
    state.taskEvents = [];
    state.taskSourceLinks = [];
    state.weeklyFacts = [];
    state.auditEvents = [];
  }
  assert.ok(state);
  const created = buildContentPackage({
    id: `seed-${kind}-${state.contentPackages.length + 1}`,
    kind,
    source: { assetIds: [`source-${kind}`], ...workflow },
    timestamp: NOW,
    workspaceId: context.workspaceId,
  });
  state.contentPackages.push(created);
