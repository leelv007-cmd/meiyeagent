...
export function contentPackageReferencesAsset(
  contentPackage: ContentPackage,
  assetId: string,
  options: {
    includeExportArtifacts?: boolean;
    includeOwnedAssetRecords?: boolean;
  } = {}
) {
  return (
    ...
    Boolean(
      options.includeOwnedAssetRecords !== false &&
    ...
    Boolean(
      options.includeExportArtifacts &&
        contentPackage.exportReceipts.some(
          (receipt) => receipt.artifactAssetId === assetId
        )
    )
  );
}

export function contentPackageRightsAssetIds(
  contentPackage: ContentPackage,
...
export function transitionContentPackage(
  ...
  if (event.type === 'provider_url_expired') {
    if (
      !contentPackageReferencesAsset(contentPackage, event.ownedAssetId, {
        includeExportArtifacts: true,
        includeOwnedAssetRecords: false,
      })
    ) {
      throw new ContentPackageTransitionError(
