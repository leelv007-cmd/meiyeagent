...
describe('creative work lifecycle', () => {
  ...
  it('issues and consumes a one-time ApprovalReceipt for the exact video contract', async () => {
    const { repository, service } = setup();
    const work = await service.createCreativeWork(owner, {
      autoConfirmBrief: true,
      briefDrafts: { audience: '附近顾客', scene: '门店实拍', tone: '可信' },
      intent: '生成一条门店服务视频',
      mode: 'direct',
      operation: 'video.generate',
      sessionId: 'session-video-approval',
      sourceReferences: [],
    });
    ...
    const receipt = await service.approveCreativeGeneration(owner, {
      approvalKey: 'approval-video-1',
      contract: videoContract,
      workId: work.id,
    });
    assert.equal(receipt.status, 'approved');
    assert.equal(receipt.binding.actionKind, 'high_cost_generation');

    await service.confirmVideoContentPackage(owner, {
      ...
      referenceAssetIds: [],
      shots: [{ id: 'shot-1', prompt: '门店服务过程' }],
      storyboardRevision: 'storyboard-v1',
      storyboardVersion: 1,
      workflowId: 'video-workflow-approved',
      workspaceId: owner.workspaceId,
      workId: work.id,
    });

    const state = await repository.loadWorkspace(owner.workspaceId);
    assert.equal(state?.creativeGenerationApprovalReceipts?.[0]?.status, 'consumed');
    assert.equal(
      state?.creativeGenerationApprovalReceipts?.[0]?.events.at(-1)?.type,
      'consumed',
    );
    assert.ok(
