...
function operationContext(context: P1Context) {
  return { ...context, actor: context.actor ?? ('owner' as const) };
}
...
export class OperationsFoundationModule implements P1OperationModule {
  ...
  constructor(
    ...
    options: { adminActorIds?: readonly string[] } = {}
  ) {
  ...
  async execute(args: {
    ...
    switch (action) {
      ...
      case 'cancel_content_package': {
        ...
        return merchantContentPackage(
          await this.operations.cancelContentPackage(
            context,
      ...
      case 'revoke_content_package_rights': {
        ...
        return merchantContentPackage(
          await this.operations.revokeContentPackageRights(
            context,
            parsed.data.packageId,
            parsed.data.reason
          )
