...
test('legacy promotion window candidates expose only matching current heads', async () => {
  const ledger = new MemoryStoreFactLedger();
  const legacySource = {
    kind: 'import' as const,
    referenceId: 'legacy-profile',
  ...
  for (const input of [
    {
      factId: 'missing-group-buy-window',
      kind: 'group_buy' as const,
      expiresAt: null,
    },
    {
      factId: 'missing-discount-window',
      kind: 'discount' as const,
      expiresAt: null,
    },
    {
      factId: 'open-ended-standard-price',
      kind: 'price' as const,
      expiresAt: null,
    },
    {
  ...
  await ledger.append({
    ...baseInput,
    factId: 'superseded-missing-window',
    kind: 'discount',
    key: 'service.superseded-discount',
    source: legacySource,
    expiresAt: null,
    expectedRevision: 0,
  });
  ...
  await ledger.append({
    ...baseInput,
    factId: 'revoked-missing-window',
    kind: 'group_buy',
    key: 'service.revoked-group-buy',
    source: legacySource,
    expiresAt: null,
    expectedRevision: 0,
  });
  await ledger.append({
    ...
    source: { ...legacySource, referenceId: 'revocation-confirmation' },
    expiresAt: null,
    expectedRevision: 1,
  });
  await ledger.append({
    ...baseInput,
    factId: 'other-store-missing-window',
    kind: 'discount',
    key: 'service.other-store-discount',
  ...
  await ledger.append({
    ...baseInput,
    factId: 'other-workspace-missing-window',
    workspaceId: 'workspace-b',
    kind: 'discount',
    key: 'service.other-workspace-discount',
    source: legacySource,
    expiresAt: null,
    expectedRevision: 0,
  });
