...
test('one Skill invocation settles two child effects independently and replay does not duplicate either settlement', async () => {
  ...
  assert.deepEqual(
    effects.map((effect) => ({
      ...
      budgetReservationCents: effect?.budgetReservationCents,
...
test('SkillDeployment maps one canonical frozen revision to a provider native version without replacing product truth', async () => {
  ...
  const deployment = await service.registerDeployment({
  ...
  assert.equal(deployment.skillRevisionRef, revision.skillRevisionRef);
  assert.equal(deployment.nativeSkillId, 'provider-skill-88');
  assert.equal(
...
async function createAcceptedEffectSkill(service: SkillService) {
  const skillId = 'skill.effect-boundary';
  const instruction =
