...
test('audio catalog entries stay providerless and cannot self-activate before a live probe', async () => {
  ...
  for (const operation of ['audio.speech', 'audio.sfx'] as const) {
    await assert.rejects(
      catalog.activate(operation, {
        activationEvidence: {
    ...
    assert.equal((await catalog.resolve(operation))?.activation, 'inactive');
  }
});

test('missing price, invalid probe, and inactive catalog block speech and SFX generation', async () => {
  ...
  const speech = {
    ...
    inputAssetIds: [],
    idempotencyKey: 'speech-closed-1',
  };
  const sfx = {
    projectId: 'project-1',
    revisionId: 'revision-1',
    ...
    inputAssetIds: [],
    idempotencyKey: 'sfx-closed-1',
  };

  for (const input of [speech, sfx]) {
    await assert.rejects(
      service.quote(owner, input),
      (error: unknown) =>
        error instanceof Error &&
        'code' in error &&
    ...
    await assert.rejects(
      () =>
        catalog.activate(operation, {
          ...
          modelId: operation === 'audio.speech' ? 'test-speech-model' : 'test-sfx-model',
          usageAmount: 0,
        }),
      /live activation evidence|pricing/u,
    );
    await assert.rejects(
      () =>
        catalog.activate(operation, {
  ...
  // Authorizer only trusts speech in this harness; SFX stays closed.
  await assert.rejects(
    () =>
      catalog.activate('audio.sfx', {
        activationEvidence: {
          configurationRevision: 'a'.repeat(64),
          evidenceId: `activation-probe-${'b'.repeat(28)}`,
          probedAt: '2026-07-16T09:00:00.000Z',
        ...
        modelId: 'test-sfx-model',
        usageAmount: 3,
      }),
    (error: unknown) =>
      error instanceof Error &&
      'code' in error &&
      error.code === 'ACTIVATION_EVIDENCE_INVALID',
  );
  assert.equal((await catalog.resolve('audio.sfx'))?.activation, 'inactive');
  await assert.rejects(
    service.quote(owner, { ...sfx, idempotencyKey: 'sfx-still-closed' }),
    (error: unknown) =>
      error instanceof Error &&
      'code' in error &&
      error.code === 'OPERATION_NOT_ACTIVE',
