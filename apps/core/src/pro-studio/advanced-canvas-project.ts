...
export class AdvancedCanvasProjectService {
  ...
  async getRevision(
    ...
    if (!revision) {
      return this.notFound(context, projectId, {
        objectId: revisionId,
        objectKind: 'revision',
      });
    }
  ...
  private async notFound(
    context: AdvancedCanvasContext,
    projectId: string,
    target?: { objectId: string; objectKind: ProStudioObjectKind }
  ): Promise<never> {
    const objectKind = target?.objectKind ?? 'project';
    const objectId = target?.objectId ?? projectId;
    await this.options.repository.appendAudit({
      action: `${objectKind}_access_denied` as ProStudioAccessDeniedAction,
