...
test('persists and reads rejection audit evidence through the existing workspace audit table', async () => {
  ...
  assert.match(
    queries[1]?.sql ?? '',
    /workspace_id = \$1[\s\S]*actor_id = \$2[\s\S]*action = ANY\(\$3::text\[\]\)[\s\S]*detail->>'outcome' = 'opaque_not_found'[\s\S]*detail \? 'targetDigest'/u
  );
  assert.equal(queries[1]?.values[0], 'attacker-workspace');
  assert.equal(queries[1]?.values[1], 'attacker-user');
