...
describe('product golden journey', () => {
  ...
  it('allows only trusted payment and worker actors to write privileged facts', async () => {
    const repository = new MemoryProductRepository();
    repository.grantMembership('user-a', 'workspace-a');
    const service = new ProductService(repository);
    ...
    const starterState = await service.bootstrap(merchant);
    starterState.entitlement.content.remaining = 20;
    await repository.save(starterState);
    const paid = await service.execute(
    ...
    assert.equal(paid.state.entitlement.plan, 'growth');
    assert.equal(paid.state.entitlement.content.remaining, 90);
    const pro = await service.execute(
      { ...merchant, actor: 'payment' },
      {
        type: 'apply_plan',
        plan: 'pro',
    ...
    assert.equal(pro.state.entitlement.plan, 'pro');
    assert.equal(pro.state.entitlement.image.allowance, 120);
    assert.equal(pro.state.entitlement.concurrencyLimit, 8);
    assert.equal(pro.state.entitlement.queuePriority, 10);
    assert.equal(pro.state.entitlement.supportLabel, 'priority');
    const stale = await service.execute(
      { ...merchant, actor: 'payment' },
      {
        type: 'apply_plan',
        plan: 'starter',
        eventId: 'payment-event-stale',
        effectiveAt: '2026-07-09T00:00:00.000Z',
      },
      'payment-event-stale'
    );
    assert.equal(stale.state.entitlement.plan, 'pro');
    assert.equal(stale.state.entitlement.sourceEventId, 'payment-event-2');
