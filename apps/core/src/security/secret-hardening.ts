/**
 * Startup secret hardening: refuse weak fixtures only in production/staging
 * (or NODE_ENV=production when APP_ENV is unset). Local fixture/e2e/tests keep
 * placeholders so onboarding stays low-friction.
 */
...
export function assertStrongSecret(
  ...
  if (allowsDevSecretDefaults(env)) {
    if (!value) {
      throw new Error(
        `${name} is required (even in local modes set a non-empty value or rely on process defaults). ${REJECTED_SET_HINT}`
      );
    }
    return;
  }
  if (!value) {
    throw new Error(
      `${name} is required in production/staging. ${REJECTED_SET_HINT}`
    );
  }
