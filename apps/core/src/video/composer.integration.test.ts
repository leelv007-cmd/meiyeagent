...
test('ffmpeg burns AIGC and brand watermark text together and writes both labels', {
  ...
  assert.ok(tools.available);
  const directory = await mkdtemp(join(tmpdir(), 'video-compose-watermarked-'));
  t.after(() => rm(directory, { force: true, recursive: true }));
  const clip = join(directory, 'clip.mp4');
  const output = join(directory, 'watermarked.mp4');
  await run(tools.ffmpegPath, [
    '-y', '-f', 'lavfi', '-i', 'color=c=0xD1495B:s=320x240:d=0.4',
    '-r', '24', '-pix_fmt', 'yuv420p', '-c:v', 'mpeg4', clip,
  ]);
  ...
  await composeVideo({
    ...
    implicitLabel: {
      serviceProvider: 'Meiye Content Copilot',
      serviceCode: 'meiye-core',
      contentId: 'content-integration-watermarked',
    },
    ffmpegPath: tools.ffmpegPath,
    ffprobePath: tools.ffprobePath,
  });
  ...
  assert.ok(outputStats.size > 0);
  assert.ok(probe.durationSeconds >= 0.3, `duration was ${probe.durationSeconds}`);
  assert.ok(videoStream);
  assert.equal(videoStream.width, 320);
  assert.equal(videoStream.height, 240);
  assert.equal(validation.visibleLabel, DEFAULT_AIGC_VISIBLE_LABEL);
  assert.equal(validation.implicitLabel.contentId, 'content-integration-watermarked');
  assert.equal(probe.tags.brand_watermark_text, '清风美学');
});
