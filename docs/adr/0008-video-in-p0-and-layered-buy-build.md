...
- Status: Accepted (2026-07-08; amended 2026-07-11, 2026-07-17)
...
## 2026-07-17 amendment: re-anchored to the marketing-loop authority

The scope coordinate system this ADR used — "P0 flagship" and the 保8缓4 scope lock in D5 — is superseded by the merged product authority `docs/design/beauty-marketing-agent-product-design-2026-07-17.md`, which organizes scope as five-entry launch contracts plus evidence-gated capability states (已验证 / 辅助完成 / 不可用).

Within that frame, D-027 of the merged authority now declares finished video's position: it is a **required deliverable of the Douyin and WeChat Channels variants** of the five-entry launch contracts. Wiring the already-built pipeline (`apps/core/src/video/`, issue 35 durable composed-video workflow) to the live model catalog and completing this ADR's six-question spike acceptance under a pre-registered evidence contract is the first engineering priority after the 48-hour thin shell (D-026). External "已验证" status still requires passing the capability gate; until then the 辅助完成 label applies to the finished-video capability itself, and the video-platform variants ship voiceover scripts / video scripts + shooting lists — first-class copy-layer deliverables produced by the LLM (merged authority D-028: copy and finished video are two independent deliverable layers, composed or not depending on the user's entry intent), not a degraded form of copy.

Still accepted and carried forward: D1 five-layer buy/build boundary, D2 KickArt positioning, the 2026-07-11 dynamic-catalog and authoring-switch amendments, and the durable-async-job requirements (absorbed as the merged authority's "long tasks recover durably" implementation constraint). The related-UI D4 copy 3-pick-1 policy is superseded by the merged authority's D-023 (one main recommendation, alternates expanded on demand); video single-shot + free retry remains consistent with D-023.

## 2026-07-24 amendment: compose exits the creative chain; N→1 narrowed

Per merged authority D-105, D1 layer-5 "own ffmpeg compose worker" and D5's first-frame→clips→ffmpeg-concat/BGM/subtitles pipeline **exit the creative main chain**: finished video = supplier model-native VideoArtifact; the product builds no timeline, clip concat, mixing, or light editor; FFmpeg/ffprobe survive only as invisible AssetValidation/DeliveryNormalization utilities, and ADR-0006's "video compose worker is the primary watch item" clause is superseded with it. The 07-17 "carried forward" list no longer covers layer-5 local composition. Per D-113, D5's "Quality: N→1 candidate eval" and D1 layer-2's "eval pick-best" narrow to: ④段 delivers 1 main candidate by default, no default gacha; N→1 remains only as bounded quality-gate retry (compliance/banned-terms screening kept). Per D-123, video allowances are 3/6/9 clips per tier (初级/中级/高级) plus a video top-up pack; pricing numbers are operator-tuned backend parameters. The AIGC label on model-native finished video is settled by D-118 (2026-07-24 user ruling): no first-party pixel burn-in — the AI-generation soft notice (D-117) plus the publish-confirmation reminder carry it, and publishing platforms auto-apply their own AIGC labels when material is uploaded; the 2026-07-11 "authoring switches" clause above is superseded with it.
