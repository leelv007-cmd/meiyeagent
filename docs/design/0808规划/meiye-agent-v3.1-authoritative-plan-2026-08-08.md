...
**计费口径（2026-08-09 用户拍板补充，收口 §23.3 `derived_revision.requiresRequote` 悬空项）**：

- 已提交并触发上游供应商 API 调用的单元，一律正常计费——Steering 不回滚、不退免已发生的供应商调用（与「accepted/acceptance_unknown 的 Provider 副作用不可被修改」一致）。
- 局部修改（`derived_revision`，以及 `future_step_patch` 命中已触发调用的单元时）＝修改指令生成对应的「修改对象」，该修改对象按正常生成口径计费；原对象费用不受影响。
- 商家面表述遵循 D-061（不暴露上游成本），只显示积分口径：例「封面与第 2 页将按修改重新生成并计 X 分；其余页不变不计费」。
