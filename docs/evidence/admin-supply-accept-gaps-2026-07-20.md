...
| 2 | Tri-modal dual-channel + story 30 main chain (procurement→publish→allocate→task→ledger→audit) | **GREEN** — `apps/core/src/p1/z2-accept/z2-accept.test.ts` recorded/fake; unit dualChannelReady same-CatalogModel honesty **FIXED** (F-I-01) | **GAP** live env-gated (see G-LIVE-*) |
| 3 | Publish gate: <2 qualified Deployments cannot mark multi-channel ready; single-channel no-fallback labeled | **GREEN** — core `publish-gate.ts` + admin supply overview SSR | **PARTIAL** merchant dual-end (see G-UI-MERCHANT-NO-FALLBACK) |
| 4 | D-048 interaction ban on ops main paths (no code/SQL/env/raw JSON/CLI) | **GREEN** — catalog / exception home / supply SSR unit assertions | **GAP** four-service Playwright still not default-CI green (spec exists; see G-E2E-PLAYWRIGHT-D048) |
...
| Field | Value |
|-------|--------|
| Status | **partial** (F-J-01) |
| Why | Admin supply overview labels `单渠道 / 无回退` (**green**). Merchant **model-settings** `ModelCard` mounts `channelReadiness` badge via shared paraglide keys; `modelCardView` / `ModelCardPicker` unit path also labels correctly. **Composer main path does not yet mount `ModelCardPicker`** — full dual-end selection surface still deferred to #83 journey coordination. |
| Dual-end requirement (D-069) | User selection page **and** admin must both label single-channel / no-fallback. |
| Mitigation | Gap explicit as **partial**, not silent complete. Accept suite asserts admin SSR + merchant label projection (`modelCardView` + settings badge keys); does **not** claim composer dual-end complete. |
| Related | J4 admin end done; Wave 1 F-J-01 partial; composer mount is #83. |

### G-E2E-PLAYWRIGHT-D048 — four-service Playwright ops path

| Field | Value |
|-------|--------|
| Status | **open** (spec written; default CI not green for full four-service boot) |
| Why | Spec Testing §5 calls for Playwright e2e: exception home → drilldown → safe action → audit loop with D-048 ban. Spec file exists: `mkfast-template-main/tests/e2e/specs/admin-supply-ops.spec.ts` (admin paths + forbidden control names). Current gate 4 default green = SSR/unit HTML scan of catalog, exception home, and supply control. |
| Claim allowed today | Unit/SSR D-048 ban **green**; full interactive four-service e2e still pending harness time + durable boot in CI. |
...
- Wave 0/1 + P2 code fixes landed (`fix/admin-supply-review-findings-2026-07-21` → main): F-G-01..05, F-S2-01..03, F-I-01, F-I-03, F-H-01/02, F-KZ-01..04, F-J-02..05.
- **F-I-01 FIXED** — unit dual-channel matrix same-CatalogModel honesty is in force; prior gap-list note that treated it as open is obsolete.
- **F-J-01 PARTIAL** — model-settings badge + label projection green; composer ModelCardPicker mount still deferred.
- **G-LIVE-\*** and full Playwright four-service remain **open**. #128 must not be claimed package-complete.
...
- **May claim:** same-increment **recorded acceptance harness** complete for #128 (five-gate unit/SSR suite green + this honest gap list).
- **May not claim:** production C5 live dual-channel multi-channel ready; full dual-end merchant composer no-fallback; Playwright four-service D-048 e2e default green — until corresponding gaps close and this file is updated.
- **May not claim:** "#128 whole package complete" while any open/partial gap above remains.
