...
Status: working evidence for C4 implementation `0fe71017` on
`main@d52df396`. This document is deliberately not a closeout claim. A cell is
...
| C4 canonical regeneration behavior after the expected red phase | 36 pass, 0 fail, 0 skip |
| Post-C4 controller merge-slice Harness assertions | 79 pass, 0 fail, 0 skip |
| #248/#247/#253 upstream contract and Harness assertions after rebase | 111 contracts + 170 Harness pass, 0 fail, 0 skip |
| #242 upstream persistence quality-gate assertions after rebase | 10 pass, 0 fail, 0 skip |
...
| Current fresh Core full suite with provisioned business PostgreSQL and separate DBOS PostgreSQL | 2533 pass, 0 fail, 10 skip, exit 0 |
...
main contains the #244 validity fixes, #242 App Shell provisioning gate, and
#253 A due-delivery chain. The current run used
`scripts/ci/provision-test-db.sh` to create a fresh business database plus a
different DBOS system database. The resulting 2533/0/10 run on the code tree at
`main@40e8efa9` plus the C4 tail supersedes the old counts as the current
full-suite verdict. The 10 skips are explicit paid/live provider opt-ins. Both
temporary databases were deleted and their post-run residual count was zero.
The subsequent rebase to `main@d52df396` added only a runbook sentence and did
not change the tested code tree.
...
| Transformation audit | Existing NotePlan rewrite emits `note_page_regenerated` in selection audit signals | #248's canonical strict contract is imported from `@meiye/contracts`; no second schema exists | Text and image consistency rewrites are constructed through that schema with `trigger: check_violation` | Production NotePlan rewrite is reachable through `UnifiedHarnessStagePorts` | Real text, image, and combined rewrite behavior is covered | The canonical signal enters the workflow execution-selection decision trace | PASS |
...
1. The main branch contains #248 M1.5's owner-only
   `note_page_regenerated.trigger` contract. C4 now imports that canonical
   schema and records `check_violation` for both production consistency rewrite
   shapes without inventing a second event truth.
...
5. The current fresh Core/PostgreSQL/DBOS full suite is green after rebasing the
   #248 M1.5, #247 mechanism, #242 provisioning, and #253 A owner changes.
   Historical red or pre-owner green counts are not reused as the current
   verdict.
