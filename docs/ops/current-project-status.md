...
- **旧仓 `legacy-web-repo` 已归档只读，勿删**：397 issue（票据史）、29 PR、CI 运行史都在那里，文档里的票号与证据链接全指向它。**该仓另由用户主动设为 private（08-17，用户确认系本人操作，非异常、非归档副作用）**——因此这些链接仅属主登录后可见，匿名访问 404 是预期行为，**不要当作残留限制去「修复」或改回 public**。
...
- **口径裁决（2026-08-17，用户）：「新文档不写旧账号名」维持不变**，不随一号恢复而调整——文档一律继续用中性名（legacy-origin-a / legacy-origin-b / legacy-web-repo）。本节至此无待决项。
