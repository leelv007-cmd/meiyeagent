...
| 项 | 定案 |
| --- | --- |
| 仿写 JSON | `xhsViralRewrite` → Langfuse `harness/xhs-viral-rewrite` |
...
| 消费点 | `notePlanInstructionsForViralAdapt` + `ModelSupplyNotePlanStructuredPort.plan`（paste marker 时注入 rewrite 系统提示） |
...
| 取材 | 粘贴正文进 rawInput；可选 `viral_reference_image` 图槽（非抓取） |
...
| 补问取材 | 粘贴正文 + 可选参考图；OpenCLI 位保留且 disabled | `viral-adapt-sourcing-card.tsx` |
| 确认卡 | 明示取材方式 + 规格 + OpenCLI 诚实状态 | `viral-adapt-confirm-card.tsx` |
| 提交意图 | marker + 粘贴正文 → Composer intent | `viral-adapt-journey.ts` + `composer-home.tsx` |
| Core 仿写→note | rewrite prompt 注入 note plan；carrier=note 投影 | `viral-adapt.ts` / `note-plan-structured-port.ts` / `unified-media-stage-ports.ts` |
