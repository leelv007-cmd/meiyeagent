...
| **A-4** `TUZI_MEDIA_BASE_URL`/`TUZI_MEDIA_API_KEY` | 图/视频容灾通道（D-129：方舟直连主、tuzi 容灾） | tuzi relay key——**可暂缓**，缺席时按既有合同投影 single_channel/no_fallback，不阻塞任何票 | 单通道投影（既有） | ◐ 未核销 2026-07-25T17:51:39.521Z（真实图片探针受理前认证失败、费用 0；如实投影 `single_channel/no_fallback`；证据：`.scratch/provisioning-live-2026-07-25/remaining-provider-live-receipt.json`） |
...
| 键 | 用途（消费方） | 需你提供 | 缺省 fixture 档 | 状态 |
|---|---|---|---|---|
| **B-1** `RESEND_API_KEY` ＋发信域名 | 注册邮件与通知（装配门、注册承接票） | Resend API key＋一个能改 DNS 的发信域名（域名验证步骤我可以给） | 邮件落日志不真发（既有） | ◐ 未核销 2026-07-25T17:52:27Z（注册邮件真实发送探针因 `tqai.uk` 域名未验证而受理前拒绝，未宣称到达；证据：`.scratch/provisioning-live-2026-07-25/resend-live-receipt.json`） |
