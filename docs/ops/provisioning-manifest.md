...
| **C-1b** 档位月价（初级/中级/高级） | 公开定价页与 Landing（两页同源读 `VITE_GROWTH_*_AMOUNT_CENTS` 支付配置）；两页确实读同一个键这件事，由 `specs/public-plan-price-source.spec.ts` 在真浏览器上看住 | **可后补**——试点期线上支付未开放（D-124），价格为意向价 | D-123 中级 ¥399/月（`VITE_GROWTH_MONTHLY_AMOUNT_CENTS=39900`）；初级 D-123 参考 ¥199、高级 ¥699 当前按「按需开通」呈现 | ⚠️ 与现状不符，待哥拍（#242）：代码与 `.env.example` 现在是 `199900`，也就是两个公开页面上写着 ¥1999/月。2026-07-28 的 `918007da`「refine landing page experience」把 39900 改成了 199900，这张表和 D-123 都没跟着动。哪个是真价请拍板；在拍之前，商家看到的是 ¥1999 |
