...
> 历史基线（2026-07-15）：本报告不得作为当前绿灯证据。2026-07-16 复核已确认 ContentPackage 代码已在 `fd78dec` 等提交落地；当前 `pnpm typecheck` 通过，但 `pnpm check` 在 Biome 阶段仍有 11 个格式问题，且最新 Pro Studio 两线口径由 ADR-0012 与 rev2 规格覆盖本报告未涉及部分。

> 复核方式：`grill-with-docs`（以 `/grilling` + `/domain-modeling` 的事实核验、权威顺序与领域语言规则执行）。
>
> 复核基线：当前 `HEAD` `233327e`（其前置文档落地提交为 `81243c2` / `812c61a`）。本轮覆盖 ContentPackage 规格、22 张执行票、两套决策票据清单、当前代码/测试，以及旧 P0/P1/UIUX 快照；代码行为未在本轮改变。

## 一、当前权威顺序

1. 当前 `HEAD` 与当前代码/测试事实。
2. [`CONTEXT.md`](../../CONTEXT.md)：产品术语、完成定义、一级导航与当前权威规则。
...
5. [`ContentPackage 实施规格`](../specs/contentpackage-productization-spec.md)：当前决策在成品聚合、状态机、迁移、管理面和验收上的工程落地口径。
6. [`ContentPackage 实施地图与机器清单`](../../.scratch/contentpackage-productization/MAP.md)：22 张票、依赖、关闭证据和 gate 的唯一执行真相；旧 UIUX 清单仅作历史对照。
...
| 决策 | 当前决策口径 | 当前实现/证据状态 |
| --- | --- | --- |
| D01 | P1 完成必须包含一条真实 provider 端到端商家链路并留证 | 未完成；真实跑通链路数仍为 0 |
| D02 | UIUX 评分冻结 6.50，不再以追分代替产品化 | 已生效；评分不是能力完成证明 |
| D03 | 北极星为真实跑通链路数 | 已写入 `CONTEXT.md`；当前值 0 |
