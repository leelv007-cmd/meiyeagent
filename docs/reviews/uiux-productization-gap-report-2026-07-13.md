...
> 状态：历史决策输入。Path B 方向仍由 ADR-0010 约束，但旧执行票已由 2026-07-15 提交行政关闭；当前产品决策以 CONTEXT.md、ADR-0011 和阶段决策日志为准。
...
- **期望**：成熟产品克隆即可体验生成（CreatOK 打开即出图 `references/benchmark/…/01-creatok-core-journey-audit.md:179`）；仓库自身声明的 fixture 走测模式应在 `APP_ENV=e2e` 下跑通 Job→Asset→Content 主链路并明确标"本地可用"（`docs/evidence/browser-dogfood-2026-07-13/report.md:37-40`）。
