...
39 张票中 24 张无阻塞可立即开工，15 张有依赖边。跨 spec 的硬依赖有两条：#376（Recipe 发布成功面板）等 #359（工具引用透传），#382（商家旅程 e2e）等 #362（Skill 治理白名单）——否则测的是数据丢失后的世界。同 spec 内的顺序边多为「同一文件/同一分派点」的语义防撞，例如 #366 等 #365（共用 auth catch-all 的 pathname 分派层）、#361 等 #359（共用三态合并语义的唯一定义处）。
