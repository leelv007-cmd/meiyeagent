...
把 `docs/reviews/admin-config-audit-2026-08-06.md`（§六 D1–D9 决议 + §五 P0/P1/P2）与 `docs/design/admin-reui-restyle-plan-2026-08-06.md`（落地状态 7 项遗留）里的全部开放待办，整理成 8 份实施 spec；对抗式复核又派生出第 9 份（I）。DRAFT——用户过目后再发布为 GitHub issue 并打 `ready-for-agent`。
...
| Spec | 主题 | 覆盖来源 | 优先级 | 备注 |
|---|---|---|---|---|
| A | 管理面权限收口 | §2.8 封禁窗口、D7、报备②（§4.5 fail-open 已证伪，降为 P2 纵深） | P1 | 安全批 |
| B | 静默数据丢失三处 | §2.7 toolEntryRefs、§2.7 Recipe 草稿、§2.5 Skill 白名单 | P0 | 数据正确性 |
| C | 供给闭环修复 | §2.2 轮换/测试连接、§2.4 模式键、§3.2 错标、D6 | P1 | 含 D6 去重 |
| D | 配方治理收敛 | §2.6 recipe-studio、§4.3 双入口、D3、D5 | 中 | recipe-studio 下线 |
| E | 商家选用技能旅程 | §2.5 user_selected、D4 | 中 | 前置 wayfinder |
| F | 异常首页真实化+入口 | §2.12、§4.1/4.4、D2 第二步、D9 | 中 | 入口与数据同批 |
| G | IA 拆页与治理清理 | §2.9/10/11、§4.2、P2、报备① | 低 | 可拆多小票 |
| H | 换装遗留与 UI 缺项 | 换装方案 7 项遗留 + 可选部件 | 低 | 复用 ReUI block |
| I | Recipe 评测证据服务签发 | 复核对 D 的裁决派生（非原始文档待办） | 中 | D 的解锁条件 |

## 依赖关系

- B 的 `toolEntryRefs` 透传是 D5 的硬前置；B 的 Skill 白名单修复是 E 的前置（否则 user_selected 绑定仍落空）。
...
B（唯一 P0）先行 → A/C 并行 → F 异常首页 → D+I 同批 → E（wayfinder 先）→ G/H 清理与补齐并行。
