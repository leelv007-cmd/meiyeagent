...
付费单店商户当前"看起来"有一个能生成图文与视频的内容产品，但真实价值链没有接通，产品站在 L1 顶端、向 L2 的门槛前止步——**真实商家用真实模型端到端跑通计数已由 `docs/evidence/contentpackage/` 登记为 1（real-run-0002）；此前长期为 0 的历史状态见 2026-07-15 及更早审计**。从商户视角，断裂具体表现为：
...
发布口径遵守 ADR-0009 单发布闸 + D01 硬 Gate：ContentPackage 六建设面一起通过才发布，不发半产品；至少一条 must-have 商户旅程用真实 provider 端到端跑通并留证，才算功能完成。北极星为"真实跑通链路数（当前 = 1）"；首条连续且满足同一聚合要求的商户旅程证据见 `docs/evidence/contentpackage/real-run-0002/journey/`。UIUX / 评审 / recorded 完备性仍不能单独替代真实跑通证据。
...
- 北极星 = 真实跑通链路数（当前 = 1）；证据：`docs/evidence/contentpackage/real-run-0002/`。UIUX / 评审 / recorded 完备性仍不能单独替代真实跑通证据。
