...
## 4. 金额 vs 条数口径分析（**须用户拍板 —— `DECISIONS.md` D1**）
...
4. **不受 transcript 清空影响**——它挂在 composer-home 的组件树上，不进 `session.transcript`。
...
### 6.2 **重大发现：D-164⑥ 决定 C 与 D-109 存在正面冲突，须用户裁定**
...
**因此本票的实现口径**：拒绝反馈文案 = 「已取消，本次没有消耗额度」，并在代码注释里写明依据 D-109 + D-164⑥C 的这次裁定。**此项须用户确认**（建议登记为 `DECISIONS.md` D5）。
...
若用户仍拍板「全拦」，实现是同一段代码里把一个常量从 `'existing_gates'` 改成 `'all_generative'`，见 §7.3。
...
**③ 票下评论**：合入前在 #261 下留一条，记「v1 取哪个口径、依据哪段决策原文、切换点是哪个常量、待谁拍板」，与 `DECISIONS.md` D2 互相引用。
...
| # | 项 | 登记位 |
|---|---|---|
| **D1** | 金额 vs 条数（本文 §4，建议：条数） | `DECISIONS.md` D1，**PENDING** |
| **D2** | 触发条件（本文 §7，建议由「全拦」修正为「零新增拦截点」） | `DECISIONS.md` D2，**PENDING**，建议按本文 §7.1 更新建议项 |
| **D3** | 反馈形态（本文 §5，建议：消息尾行挂卡的原位） | `DECISIONS.md` D3，**PENDING** |
| **D5（新增）** | 规划成本口径：D-164⑥C 与 D-109「Planner 进 ProviderCost、不重复扣用户」冲突，本产品拒绝时商家侧真实消耗为 0（本文 §6.2） | **建议新增到 `DECISIONS.md`** |
