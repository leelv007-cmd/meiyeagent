...
| V31-47 | [跨载体交付真接线（一 Make 一载体），并拆除 freeze 处的 fail-closed 门](V31-47-cross-carrier-execution-wiring.md) | implemented (2026-08-11, FIX-P1-01 wiring landed; residual: per-carrier ledger split / V31-59) |
...
| V31-52 | [一键授权后「已保存到素材库」确认文案 60s 不出现（共享 fixture 上游断言）](V31-52-inline-authorize-saved-copy-never-appears.md) | fixed (local; product durable ready copy + product.execute; interaction 2/2; full browser residual) |
| V31-53 | [goal-proactive 旅程用浏览器注入 gate config，服务端 `.strict()` 拒绝](V31-53-goal-proactive-client-injected-gate-config.md) | fixed (local; admin-config seam + owned-data now; Chromium 3/3 @ 2026-08-11 handoff; schema not relaxed) |
| V31-54 | [K 自报旅程被 `case_image` source slot 挡在门口，`5ed00f453` 的浏览器验证腿至今未跑过](V31-54-k-journey-case-image-slot-fixture-gap.md) | fixed (local; seedComposerInlineAuthorize on real path; unit/static; full browser residual for 5ed00f453 leg) |
