...
| V31-91 | [显式 start 间歇性 409 COMPOSER_PLAN_START_FAILED：确认落库与 /start 之间存在竞态](V31-91-composer-plan-start-409-race.md) | in progress（2026-08-16）— ①可区分已落地：十五处裸抛改为十五个码（下次红即可读出是哪一支）；②定位竞态方、③不加重试 未动 |
...
| V31-96 | [WorkbenchCreateLayout 换根元素类型，session.phase 每次跨界就重挂整个 Composer](V31-96-workbench-create-layout-reparents-composer.md) | open（2026-08-15）— 根因已定位且已核实。**由「可选清理」升为必需**（2026-08-15 晚更正）：V31-93 修完「面板被销毁」那一支后，带修复的树上仍复现同一形态，判定为**点击在到达 handler 之前丢失**——提升状态救不了，只有停掉重挂能解。动的是布局合同，须先拍板再实施 |
