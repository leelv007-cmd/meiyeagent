...
|---|---|---|---|---|---|---|---|
| AC1 | — | — | — | — | — | — | — |
| AC2 | `apps/core/src/p1/agent-session/composer-plan-session.ts:956`（`approvalBasisForSubmission` → pure copy = `policy_exempt_copy`） | `apps/core/src/p1/harness/task-admission.ts:579`（`policy_exempt_copy` 立即 admit，无 merchant decision） | `apps/core/src/p1/agent-session/composer-plan-session.test.ts:1041`（copy → `policy_exempt_copy`）；`execution-plan-admission.test.ts:138`（exempt 仍冻结） | — | — | **2/2 pass** @ tip `1955a278e`（`v31-level1-copy-journey.spec.ts`；e2e-lock PORT=3211；`/tmp/v31-residual-reverify/pw2-level1.log`）。resume clean PORT=3180 同绿 | `v31-browser-acceptance`（`run-v31-browser-acceptance.sh:26`） |
| AC3 | `mkfast-template-main/src/product/composer/composer-home.tsx:4285`（`composer-quote-line` 常显）＋ `:3993`/`:4375` shortfall | `mkfast-template-main/src/product/composer/workbench-credit-purchase-actions.tsx:15-26`（booster / upgrade 双出口） | `quota-blocking-card.interaction.test.tsx:240`；level1 spec 真重放不双扣（`:267` case） | — | — | **同 AC2：2/2 pass** @ tip `1955a278e` | `v31-browser-acceptance` |
| AC4 | — | — | — | — | — | — | — |
| AC5 | — | — | — | — | — | — | — |
...
- Playwright 轴对 AC2／AC3 **有真绿**（resume + residual reverify @ `1955a278e`）。unit/eval 与 PG 轴本轮未在 tip 定向回填数字 ⇒ 结果格仍有 `—` ⇒ **按填表规则不得勾选** AC2／AC3。
