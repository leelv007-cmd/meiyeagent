...
### 1. make-steering 两端 task_id 拼法不同：`queued_steer` 永远 drain 不到 Make（B 已修，`4ebeb09df`；A 已修 V31-107）
...
**已修：`eb9accf71` ＋ `4044cd7e4` ＋ `64c196143`** — 定性为候选 (b)：`sendHarnessMediaJobTerminal`（`apps/core/src/p1/harness/dbos-workflow.ts:2596-2620`）是本文件里**唯一**不走 `HarnessRuntimeIdResolver` 的 DBOS send——同文件的 `resumeHarnessDbosWorkflow:2492`、`resumeHarnessDbosInteractionWorkflow:2529`、`abandonReleasedHarnessReservation:2546`、`createHarnessInterruptResumeBridge:428` 四处都写 `resolver?.workflowRuntimeId(...) ?? harnessRuntimeId(...)`，只有媒体回送直接拿冻结提交里的 `correlationId` 推地址。于是「resume 走得通、媒体回送走不通」正是这条不对称：resume 打的是准入登记的 runtime id，媒体回送打的是 `correlationId` 拼出来的 id。修法＝新增 `harnessMediaTerminalDestination`：`correlationId` 推出的 id 在 DBOS 里活着就仍投给它，只有当它哪儿都没登记时才回落到准入登记的 runtime id（生产接线 `apps/core/src/assembly/core-assembly.ts:1333` 传 `harnessSchemaStore`）。单向性是刻意的——`workflowRuntimeId` 会返回 `sourceTaskId` 匹配到的**最新**一行，无条件采信它会把冻结在前驱上的媒体结果改投给改价/迟答后继，把今天能走通的投递弄坏；第二条单测钉住这个方向。红→绿证据：新增 `apps/core/src/p1/harness/media-terminal-destination.dbos.postgres.test.ts` 在真 DBOS 上以 `Sent to non-existent destination workflow UUID: harness.v1:…` 原样复现（修前红 31.5s／修后绿 0.4s）。
...
验证：rights-revocation 限速修前 3/3 红→修后 3/3 绿、裸跑 2/2 绿；共享 helper 的 living-plan/artifact-growth/context-fence 各 1 轮绿、p2-browser-closure 3/3、m04 6/6；`src/product/composer` vitest 315 测试全绿；已答记忆作用域两条行为测试（新 revision/新 requestId 仍渲染卡、提交失败还卡）修前 1 红修后 18/18；tsc/biome 绿。

**限速复现配方（本族及一切「断言读瞬态 UI」的红适用）**：在被测 helper 的关键点击**之前**开、断言完立刻关（全程开会烧光 420s 预算）；`rate: 20` 是实测一轮出红档，OS 级 CPU 压载（16 路 `yes`）无效——它卡的是渲染线程节奏，对网络/DB 类红无效；chromium only：
