...
- [x] 注入清单可见且撤销后不再注入（Playwright §37.4-B2）
...
| AC3 | `apps/core/src/p1/operations/postgres-memory-injection-receipt.ts:72`（`save`，put-once＋payload 同一性校验）；vault 投影 `reuse-memory-service` / `listMemoryEntriesPage` join preference head `recordState`（`f217c2c92`） | `mkfast-template-main/src/product/memory-injection-receipt.tsx:65`（清单面板）/ `:43`（`action: 'revoke_memory'` 撤销）；`memory-vault-page.tsx` status 展示 | `postgres-memory-injection-receipt.postgres.test.ts:28`（put-once＋重启可读）；`agent-memory-platform.test.ts:445`（撤销后不再注入）；`reuse-memory-service.test.ts` revoke→vault `revoked` | `14/14 pass`（agent-memory-platform @ `a94520ee1`）＋ tip residual reverify memory unit **18/18** `/tmp/v31-residual-reverify/core-memory-unit.log` | `1/1 pass, skip 0`——**集成树 `a94520ee1`** 一次性库（说明 ⑤） | **1/1 PASS**（INT tip `1955a278e`，e2e-lock PORT=3211 CORE=4211；`v31-memory-injection-b2-journey` revoke 后仅撤销项停注，1.5m；`/tmp/v31-residual-reverify/pw2-b2.log`）。产品根因已由 `f217c2c92` 修：promotion-only 曾把 vault 标 `confirmed` 忽略 head `recordState=revoked`。首轮 PORT=3205 `CORE_UNAVAILABLE` 为 stack 退化，不记产品红 | `core-persistence`（PG 面）＋ `v31-browser-acceptance`（门脚本跑 B2） |
...
| AC3 | **满足**（unit/eval 绿；PG `1/1`；Playwright B2 **1/1** @ tip `1955a278e` + `f217c2c92`） | **已勾** | residual reverify handoff `docs/handoff/v31-wave4-residual-reds-report-2026-08-11.md`；**不得**用 mid-stack `CORE_UNAVAILABLE` 抹掉产品绿 |
