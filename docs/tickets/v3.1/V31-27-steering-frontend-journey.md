...
**Status**: merged-with-evidence-debt (merged aaad2a0f1, 2026-08-09) — Wave-4 浏览器实证证伪 AC1（`v31-mid-run-steering-journey` 2 FAIL，红在前置步骤，本票被测行为未被走到）；降级为主控 2026-08-10 裁决，口径同 V31-18；**2026-08-23 补：前置步骤的红已修（V31-90 同轮，集成分支 `claude/advisory-integration`），`v31-mid-run-steering-journey` 本机锁内整文件 2/2 绿两轮，CI 证据待 Advisory telemetry 同 SHA 绿后回填**
