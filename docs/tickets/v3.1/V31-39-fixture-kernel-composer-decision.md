...
- **Wave-4 归档：4B 修复归本票（`1d5d43a03`，2026-08-10 review-memory 落）** ⇒ `fix(agent-session): fail the run when a turn produces neither a plan nor a question`。commit message 逐字点名本票：一个既不提方案也不问商家就结束的 turn，**留下的东西永远推不动这个 run**。`assertTurnCanBeWaitedOn` 本来就拒这种情况，但**抛出发生在状态更新之前**，于是 run 自己的 status 永远停在 `running`。修法＝先把 run 标成 `failed`（带真实抛出的错误）再重抛。**没有新增失败机制、没有计费/结算改动**——因为这次抛出发生在任何积分被动过之前（planning 现在跑在 `store.claim()` 之前，即 V31-18 P0-1 的原子序）。改动面：`composer-plan-session.ts` +15/−1。**范围刻意收窄**在「decision 存在、但既不 propose 也不 ask」这一种（`assertTurnCanBeWaitedOn` 的抛出点）；`null`-decision / `systemOnlyBlock` 的早退**未动**——代码自己的注释写明那是一份**刻意的、独立的**合同，不是本修复的目标。这条自我限定值得保留：下一个读它的人不要把两种早退合并处理。
