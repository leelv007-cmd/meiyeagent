...
- Wave 4 B2 实施 lane（2026-08-11）：receipt 版本保持 v1，entry 上新增 `source` 为 backward-compatible optional，`preview` 契约上限 500 字符；来源只在查询时由 preference 的 `candidateId` 追到既有 memory-vault source ledger，**不改写 put-once receipt 也不进入 business identity／冲突判定**。历史 v1 行缺 `source` 时 UI 安全回落「来源对话暂不可查看」，memoryId 仅作次要信息；来源删除后动态投影只返回 `deleted: true`，不再携带 preview／observedAt。本 lane 按派件只跑 focused unit/Web、quality gate、typecheck/check 与 Playwright `--list`，不宣称已实跑浏览器旅程。
