...
- [ ] 「没有门店」的编码在**契约层**被定下（zod schema 或投影函数的返回类型），并在票下写明选了 `null` 还是缺席、以及为什么
- [ ] `v31-day0-free-creation-journey.spec.ts:109` 按裁定的编码断言并转绿
- [ ] 该断言**能区分「明确没有门店」与「投影没跑/字段改名」**：具体做法是同一处再断言一个正向事实（投影确实执行过的证据）。**只改一侧不算过**
- [ ] **变异反证**：让投影在门店查询前早退（或把字段改名）⇒ 该断言必须转红。改后立即还原，终态 `git status --porcelain` 空
- [ ] day0 spec 整条绿（round3 基线为 `1 failed`）
