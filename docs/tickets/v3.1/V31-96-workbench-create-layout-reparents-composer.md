...
**Status**: 已关票（2026-08-16，`0c54507be` 经 PR #10）— `required` 同 SHA 绿；`assertThreeModalDiscovery` 已 4 轮 `--retries=0` 绿；1000px/390px 客观项本机真机已验（`touch-action` 在真层叠里确为 `auto`），**观感经用户拍板维持现状、1240 门槛不动**；⚠️ 遗留一条已知缺口：新增的窄视口 spec 至今**未在 CI 执行过一次**（每轮都被 workerd 仪器故障截断落进「did not run」，归 V31-70）
...
不过中间那一跳靠的是规范而不是指望，做核对的人可以据此判断权重：样式表里的
author `!important` 声明在层叠里**排在普通 inline 声明之上**（CSS Cascade 4 §6.1：
先按 origin/importance 排序，`style` 属性那条「特异性高于任何选择器」是在**其后**
才参与比较的）。库写的是普通 inline `touchAction`，我们写的是带 `!important` 的
author 规则，所以赢是规范定的。真机那一跳要验的是**这条规则确实命中了那个元素**
（类名挂对、CSS 真的进了包），不是层叠本身会不会赢。
...
- [x] 1000px 与 ~390px 真机观感核对（CI 只跑 1440）—— **客观项已验**（2026-08-16，本机真浏览器）；
      **主观观感已由用户拍板（2026-08-16）：维持现状，1240 门槛不动**，理由见下节末
...
### 拍板结果（2026-08-16）：维持现状，`WORKBENCH_DUAL_COLUMN_MIN_WIDTH_PX = 1240` 不动

用户已拍板：1000px 掉单栏可接受，门槛保持 1240，**本项到此为止，不再欠**。

支撑这个决定的机械理由（**是推论，不是实测**，须照此读）：右栏地板是 24%。
1440 视口下实测 group 只有 1080px（左右各内缩约 180px）。若该内缩是固定值，
1000px 视口的 group 约 640px，24% ≈ **154px** —— 放不下一张作品卡。
1240 这个门槛很可能正是为避开该区间而定。

**为什么这只能是推论**：双栏在 <1240 下根本不渲染，
所以在不改门槛的前提下**无法量到** 1000px 的 group 宽度。
若日后真要降门槛，必须同时重定 40%/24% 两个地板（见 V31-99 的实测方法），
那属于新票，不是本票欠账。
