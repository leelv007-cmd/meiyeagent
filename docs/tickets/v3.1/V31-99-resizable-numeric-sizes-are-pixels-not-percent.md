...
**Status**: 已合入（2026-08-16，`f15eb4c3` 经 PR #17 进 main `6a4f733ae`）— 四处改为显式 `%`；两个待答问题都已查实（数字无规格出处；**从未部署过，不存在已习惯窄栏的商家**），故抬地板不构成对现有用户的行为回退；**真机拖拽两端极限已实测通过（40%/24% 皆生效，见「真机拖拽实测」）**
