...
> **D1/SQLite 描述已过时（ADR-0006）**：本页的 D1、`provider: 'sqlite'`、`env.DB` 均为模板原文。当前实现：`drizzleAdapter(getDb(), { provider: 'pg' })`（`src/auth/auth.ts`），Better Auth 表存于托管 PostgreSQL，Workers 经 `HYPERDRIVE` 绑定访问（`src/db/index.ts` 使用 `env.HYPERDRIVE.connectionString`）。以代码与仓库根 `docs/adr/0006-p0-runtime-topology.md` 为准。
