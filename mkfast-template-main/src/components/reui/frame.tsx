 ...
 *
 * Upstream ReUI ships this file with a few truncated class fragments (mobile
 * radius/spacing overrides cut mid-token); those dead fragments are removed
 * here. The admin surface is desktop-only, so nothing is lost.
 */
const frameVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      // 0.5 (2px): it is the practical floor.
      spacing: {
...
type FrameHeadingLevel = 2 | 3 | 4 | 5 | 6;

// Frames are the app's section containers, so FrameTitle has to be a real heading
// for screen-reader heading navigation. Level 2 is the default because every page
// owns the single h1; raise it on a Frame nested inside another titled section.
const FrameHeadingLevelContext = createContext<FrameHeadingLevel>(2);
...
function Frame({
  ...
  VariantProps<typeof frameVariants> & { headingLevel?: FrameHeadingLevel }) {
  return (
    <FrameHeadingLevelContext.Provider value={headingLevel}>
      <div
        className={cn(
        ...
        {...props}
      />
    </FrameHeadingLevelContext.Provider>
  );
}
...
function FrameTitle({ className, ...props }: React.ComponentProps<'h2'>) {
  const level = useContext(FrameHeadingLevelContext);
  // Preflight strips the browser's heading font-size, weight and margin, so the
  // element swap is invisible and the existing utilities keep driving the look.
  const Heading: `h${FrameHeadingLevel}` = `h${level}`;
