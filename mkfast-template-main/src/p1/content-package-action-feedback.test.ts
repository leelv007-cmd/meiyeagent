...
describe('ContentPackage action feedback', () => {
  ...
  it('returns dedicated feedback when three-platform generation fails', async () => {
    let refreshes = 0;

    const feedback = await recoverContentPackageAction(
      new P1RequestError(
        'Variant generation failed.',
        'CONTENT_PACKAGE_VARIANT_GENERATION_FAILED'
      ),
      async () => {
        refreshes += 1;
      },
    ...
    assert.equal(feedback, 'variant_generation');
    assert.equal(refreshes, 1);
  });
});
