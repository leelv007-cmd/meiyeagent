...
export const entitlementAddOnFormSchema = z.object({
  offerId: z.string().trim().min(1, m.p1_entitlement_validation_add_on()),
});

export const autoTopUpFormSchema = z.object({
  enabled: z.boolean(),
  monthlyCapYuan: z
    .number({ error: m.p1_entitlement_validation_amount() })
    .finite(m.p1_entitlement_validation_amount())
    .min(0, m.p1_entitlement_validation_monthly_cap_min())
    .max(9_007_199_254, m.p1_entitlement_validation_monthly_cap_max()),
});
...
export const strictByokExecutionFormSchema = z.object({
  connectionId: z
    .string()
    .trim()
    .min(1, m.p1_entitlement_validation_connection()),
  modelId: z.string().trim().min(1, m.p1_entitlement_validation_model()),
  profileId: z.string().trim().min(1, m.p1_entitlement_validation_profile()),
  prompt: z
    .string()
    .trim()
    .min(1, m.p1_entitlement_validation_prompt())
    .max(65_536, m.p1_entitlement_validation_prompt_too_long()),
});
