...
const MODEL_SECTIONS: ModelSection[] = [
  {
    id: 'llm',
    title: m.model_settings_llm_title,
    description: m.model_settings_llm_description,
    operation: 'copy.generate',
    icon: IconSparkles,
  },
  {
    id: 'image',
    title: m.model_settings_image_title,
    description: m.model_settings_image_description,
    operation: 'image.generate',
    icon: IconPhoto,
  },
  {
    id: 'video',
    title: m.model_settings_video_title,
    description: m.model_settings_video_description,
...
const MODEL_TAGLINES: Record<string, () => string> = {
  'llm-anthropic': m.model_settings_tagline_anthropic,
  'llm-domestic': m.model_settings_tagline_domestic,
  'llm-gemini': m.model_settings_tagline_gemini,
  'llm-openai': m.model_settings_tagline_openai,
};
...
function ModelStatus({ model }: { model: CatalogModelView }) {
  if (model.availabilityKind === 'local_fixture') {
    return (
      <Badge className="text-muted-foreground" variant="ghost">
        {m.model_settings_local_fixture()}
      </Badge>
    );
  }
  return model.available ? (
    ...
    <Badge variant="destructive">{m.model_settings_unavailable()}</Badge>
  );
}
        ...
        <CardDescription className="meiye-type-body text-foreground">
          {tagline ??
            (model.capabilityLabels.join(' · ') ||
              m.model_settings_capability_pending())}
        </CardDescription>
        <CardAction>
          <Button
            aria-label={
              favorite
                ? m.model_settings_unfavorite()
                : m.model_settings_favorite()
            }
      ...
      <CardContent className="space-y-2">
        <div className="flex min-h-5 flex-wrap gap-2">
          {preferences.userDefault === model.id ? (
            <Badge variant="secondary">
              {m.model_settings_personal_default()}
            </Badge>
          ) : null}
          {preferences.workspaceDefault === model.id ? (
            <Badge variant="secondary">
              {m.model_settings_workspace_default()}
            </Badge>
          ) : null}
          {recentIndex >= 0 ? (
            <Badge variant="ghost">
              {m.model_settings_recent_rank({ rank: recentIndex + 1 })}
            </Badge>
          ) : null}
          {current ? (
            <Badge variant="secondary">
              <IconCheck />
              {m.model_settings_selected()}
            </Badge>
          ) : null}
        </div>
        {!model.available ? (
          <p className="text-sm text-destructive">
            {model.unavailableReason ?? m.model_settings_unavailable_fallback()}
          </p>
        ) : null}
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            variant="outline"
          >
            {m.model_settings_personal_default_action()}
          </Button>
          {canManageWorkspace ? (
            <Button
              ...
              variant="outline"
            >
              {m.model_settings_workspace_default_action()}
            </Button>
          ) : null}
        </div>
      </CardContent>
...
export function ModelSettings() {
  ...
  const error = errorCause ? m.model_settings_load_error() : undefined;
  ...
  const execute = async (
    ...
    try {
      ...
      toast.error(m.model_settings_save_error());
    }
  };
  ...
  const selectModel = async (model: CatalogModelView) => {
    ...
    await execute(
      'record_recent',
      { modelId: model.id },
      m.model_settings_selection_recorded(),
      operation
    );
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <TabsList aria-label={m.model_settings_model_types()}>
      ...
      {MODEL_SECTIONS.map((item) => (
        <TabsContent key={item.id} value={item.id}>
          <div className="mb-4 flex items-center gap-4 rounded-xl bg-surface-1 p-4">
            ...
            <img
              alt={m.model_card_preview_alt({ name: item.title() })}
      ...
      {error ? (
        <Card>
          <CardHeader>
            <CardTitle>{m.model_settings_load_error_title()}</CardTitle>
            <CardDescription>{error}</CardDescription>
          </CardHeader>
          <CardContent>
            <Button onClick={() => void refresh()} variant="outline">
              {m.model_settings_retry()}
            </Button>
          </CardContent>
        </Card>
      ) : loading ? (
        ...
        <div className="space-y-4">
          <RadioGroup
            aria-label={m.model_settings_use_this_run()}
              ...
              <ModelCard
                ...
                onFavorite={(candidate) =>
                  execute(
                    ...
                    preferences.favorites.includes(candidate.id)
                      ? m.model_settings_favorite_removed()
                      : m.model_settings_favorite_added()
                  )
                }
                onSetUserDefault={(candidate) =>
                  execute(
                    'set_user_default',
                    { modelId: candidate.id },
                    m.model_settings_personal_default_updated()
                  )
                }
                onSetWorkspaceDefault={(candidate) =>
                  execute(
                    ...
                    { modelId: candidate.id },
                    m.model_settings_workspace_default_updated()
                  )
                }
