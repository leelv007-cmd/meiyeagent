...
export const CONTENT_MODULE_LABELS = {
  get before_after() {
    return m.content_module_label_before_after();
  },
  get package_explainer() {
    return m.content_module_label_package_explainer();
  },
  get price_card() {
    return m.content_module_label_price_card();
  },
  get review_card() {
    return m.content_module_label_review_card();
  },
  get shooting_checklist() {
    return m.content_module_label_shooting_checklist();
  },
  get social_cover() {
    return m.content_module_label_social_cover();
  },
  get store_intro() {
    return m.content_module_label_store_intro();
  },
} satisfies Record<CreativeContentModuleId, string>;

export const CONTENT_MODULE_REQUIREMENTS = {
  get before_after() {
    return m.content_module_requirement_before_after();
  },
  get package_explainer() {
    return m.content_module_requirement_package_explainer();
  },
  get price_card() {
    return m.content_module_requirement_price_card();
  },
  get review_card() {
    return m.content_module_requirement_review_card();
  },
  get shooting_checklist() {
    return m.content_module_requirement_shooting_checklist();
  },
  get social_cover() {
    return m.content_module_requirement_social_cover();
  },
  get store_intro() {
    return m.content_module_requirement_store_intro();
  },
} satisfies Record<CreativeContentModuleId, string>;
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          >
            {m.content_module_title()}
          </h3>
          <p className="mt-1 text-sm/6 text-muted-foreground">
            {presetName
              ? m.content_module_preset_description({ preset: presetName })
              : m.content_module_manual_description()}
          </p>
        </div>
        <Badge variant="secondary">
          {m.content_module_selected_count({ count: selectedModules.length })}
        </Badge>
      </div>
      <fieldset className="min-w-0">
        <legend className="mb-2 text-xs font-medium text-muted-foreground">
          {m.content_module_select_legend()}
        </legend>
                ...
                >
                  <span className="flex flex-wrap items-center gap-2 font-medium text-foreground">
                    ...
                    {!available ? (
                      <Badge variant="outline">
                        {m.content_module_unavailable()}
                      </Badge>
                    ) : null}
                  </span>
                  <span className="block text-muted-foreground">
                    {m.content_module_material_requirement({
                  ...
                  {!available ? (
                    <span className="block text-xs text-muted-foreground">
                      {presetName
                        ? m.content_module_unavailable_preset({
                            preset: presetName,
                          })
                        : m.content_module_unavailable_entry()}
                    </span>
                  ) : null}
                </label>
              </div>
      ...
      <div className="text-sm">
        <p className="text-xs font-medium text-muted-foreground">
          {m.content_module_structure_title()}
        </p>
      ...
      <p className="text-xs text-muted-foreground">
        {m.content_module_persistence_note()}
      </p>
    </section>
  );
}
