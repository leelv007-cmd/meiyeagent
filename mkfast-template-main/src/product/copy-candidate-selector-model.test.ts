...
test('sorts one complete copy batch into stable A/B/C candidates', () => {
  ...
  assert.equal(model.presentation, 'legacy');
  assert.equal(model.primaryCandidate, undefined);
  assert.deepEqual(model.alternativeCandidates, []);
});

test('uses only a trace-backed server recommendation as the primary candidate', () => {
  const decisionTrace = {
  ...
  assert.equal(model.presentation, 'recommended');
  assert.equal(model.primaryCandidate?.asset.id, 'asset-b');
  assert.equal(model.primaryCandidate?.recommended, true);
  assert.deepEqual(
    model.alternativeCandidates.map(({ asset }) => asset.id),
    ['asset-a', 'asset-c']
  );
  assert.equal(model.selectedAssetId, 'asset-b');
  assert.equal(model.canAccept, true);
  assert.deepEqual(model.decisionTrace, decisionTrace);
});

test('does not invent a primary recommendation when the id or trace is invalid', () => {
  const completeTrace = {
    complianceStatus: '已校验',
    customerAction: '私信预约',
    deliverables: ['文案'],
    expressionIdentity: '门店主理人',
    factReferences: ['本店事实'],
  ...
  const missingTrace = buildCopyCandidateSelectorModel({
    assets,
    contents: [],
    job,
    recommendedAssetId: 'asset-a',
  });
  const unknownAsset = buildCopyCandidateSelectorModel({
    assets,
    contents: [],
    decisionTrace: completeTrace,
    job,
    recommendedAssetId: 'asset-unknown',
  });

  for (const model of [missingTrace, unknownAsset]) {
    assert.equal(model.presentation, 'legacy');
    assert.equal(model.primaryCandidate, undefined);
    assert.equal(model.selectedAssetId, undefined);
    assert.equal(model.canAccept, false);
