...
export interface CopyCandidateSelectorModel {
  ...
  alternativeCandidates: CopyCandidateOption[];
  batchLabel: string;
  ...
  presentation: 'legacy' | 'recommended';
  primaryCandidate?: CopyCandidateOption;
...
export function buildCopyCandidateSelectorModel(input: {
  ...
  const recommendedCandidate = candidates.find(
    ({ asset }) => asset.id === input.recommendedAssetId
  );
  const hasRecommendation = Boolean(
    recommendedCandidate && isCompleteDecisionTrace(input.decisionTrace)
  );
  if (recommendedCandidate && hasRecommendation) {
    recommendedCandidate.recommended = true;
  }
  const selectedAssetId =
    acceptedAssetId ??
    requestedSelection ??
    (hasRecommendation ? recommendedCandidate?.asset.id : undefined);
  const presentation = hasRecommendation ? 'recommended' : 'legacy';
  const primaryCandidate = hasRecommendation ? recommendedCandidate : undefined;
  const alternativeCandidates = primaryCandidate
    ? candidates
        .filter(({ asset }) => asset.id !== primaryCandidate.asset.id)
        .slice(0, 2)
    : [];
  ...
  return {
    ...
    ...(input.decisionTrace && hasRecommendation
...
function isCompleteDecisionTrace(
  trace: CreativeRecommendationDecisionTrace | undefined
) {
  return Boolean(
    trace?.whyPost.trim() &&
      trace.expressionIdentity.trim() &&
      trace.factReferences.length > 0 &&
      trace.factReferences.every((value) => value.trim()) &&
      trace.platforms.length > 0 &&
      trace.platforms.every((value) => value.trim()) &&
      trace.customerAction.trim() &&
      trace.complianceStatus.trim() &&
      trace.deliverables.length > 0 &&
      trace.deliverables.every((value) => value.trim())
  );
}
