...
const { CreativeJobFailureNotice, CreativeSubmissionFailureNotice } =
...
const messages = {
  ...
  multiNotSupported: () => '视频生成当前仅支持一张参考图，请只保留一张后重试',
  oversized: ({ photo }: { photo: string }) =>
    `照片「${photo}」：文件大小超出参考图处理限制；请压缩后重新上传 JPG/PNG 原图。`,
  rightsIncomplete: ({ photo }: { photo: string }) =>
    `照片「${photo}」：缺少完整的素材权利证明；请到素材页补充证明或换一张授权完整的照片。`,
  submitBlocked: () => '以下照片暂时无法作为参考图，请先处理后再提交',
  title: () => '这些参考照片需要处理',
...
test('reference failure names every affected photo, explains the current gap, and gives a repair action', () => {
  for (const failureCode of [
    ...
    assert.match(html, /href="\/dashboard\/assets"[^>]*>去素材页处理照片/);
    assert.equal((html.match(/data-reference-photo=/g) ?? []).length, 3);
  }
});

test('submission reference details render each photo, Chinese reason, thumbnail, and repair action', () => {
  const html = renderToStaticMarkup(
    <CreativeSubmissionFailureNotice
      currentAssets={[
        currentAsset('asset-auth', 'withdrawn', 'receipt-1', '护理室.jpg'),
        currentAsset('asset-large', 'authorized', 'receipt-2', '项目近照.png'),
      ]}
      details={{
        referenceFailures: [
          {
          ...
          { assetId: 'asset-large', reasonCode: 'oversized' },
        ],
      }}
      messages={messages}
    />
  ...
  assert.match(html, /以下照片暂时无法作为参考图，请先处理后再提交/);
  assert.match(html, /护理室\.jpg[^<]*公开营销授权已失效[^<]*恢复授权/);
  assert.match(html, /项目近照\.png[^<]*文件大小超出[^<]*压缩后重新上传/);
  assert.match(html, /<img[^>]*alt="护理室\.jpg"/);
  assert.match(html, /<img[^>]*alt="项目近照\.png"/);
  assert.match(html, /href="\/dashboard\/assets"[^>]*>去素材页处理照片/);
  assert.equal((html.match(/data-reference-photo=/g) ?? []).length, 2);
  assert.doesNotMatch(html, /authorization_withdrawn|oversized/);
});

test('video reference limit uses the dedicated merchant-facing message', () => {
  const job = failedJob();
  job.failureCode = 'video_reference_limit';
  ...
  assert.match(html, /视频生成当前仅支持一张参考图，请只保留一张后重试/);
  assert.doesNotMatch(html, /video_reference_limit/);
});
...
test('execution mode gates explain the stop, refund, and zero provider cost', () => {
  for (const [failureCode, message] of [
    ['model_execution_disabled', '模型执行已停用'],
    ['media_execution_disabled', '媒体执行已停用'],
  ] as const) {
    const job = failedJob();
    job.failureCode = failureCode;
    const html = renderToStaticMarkup(
    ...
    assert.match(html, /产品额度已退回/);
    assert.match(html, /未产生供应商费用/);
    assert.doesNotMatch(
      html,
      /model_execution_disabled|media_execution_disabled/
    );
  }
});
