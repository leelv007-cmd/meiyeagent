...
test('locks a stable Work-bound draft with one fixed candidate per shot', () => {
  const input = {
    approvalReceiptId: 'creative-generation-approval-1',
    aigcLabelEnabled: true,
    executionContract: frozenVideoContract(['contains_face']),
    catalogModelId: 'seedance-2',
  ...
  assert.equal(
    first.payload.approvalReceiptId,
    'creative-generation-approval-1'
  );
  assert.equal(first.payload.catalogModelId, 'seedance-2');
  assert.equal(first.payload.aigcLabelEnabled, true);
  assert.deepEqual(first.payload.executionContract, input.executionContract);
  assert.deepEqual(
    first.payload.shots.map(({ durationSeconds, height, width }) => ({
      durationSeconds,
      height,
      width,
    })),
    [
      { durationSeconds: 4, height: 1280, width: 720 },
      { durationSeconds: 4, height: 1280, width: 720 },
      { durationSeconds: 4, height: 1280, width: 720 },
      { durationSeconds: 3, height: 1280, width: 720 },
    ]
  );
  ...
  assert.notEqual(
    buildVideoWorkflowDraft({
      ...input,
      approvalReceiptId: 'creative-generation-approval-2',
    }).payload.workflowId,
