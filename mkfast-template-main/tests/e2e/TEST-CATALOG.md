...
| 4 | canonical handoff ready cache never crosses logout from account A to account B | A 账号真实交付后打开 canonical one-shot URL 并看到 ready 文案；同一 document/QueryClient 内从产品 UI 退出 A、登录 B，再以 SPA 导航打开同 URL；必须产生新 `assisted_consume_handoff` server response（`consumed|not_found`），只显示 unavailable，不得命中 A 的 ready cache/文案。 |
