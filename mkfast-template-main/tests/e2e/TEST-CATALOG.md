...
| # | Test name | Flow |
|---|---|---|
| 1 | A limited-time price is quoted inside its window and dropped once the clock passes it | Seed a legacy StoreProfile with one confirmed project, open the store wizard, and state a price together with the day it runs to — the save button stays disabled until that question is answered, so no price can reach the ledger without it. Read the sent `finalize_store_intake` and require the window to ride on the price candidate's own `expiresAt` and to be repeated identically on the profile side. Freeze a ContextBundle through the production `context_bundle_compile(at)` replay/audit seam while the window is open and require price revision 1 in `referencedFactRevisions`. Recompile the same bundle just after `expiresAt` and require: `store_fact_history` still contains exactly price revision 1 (nothing was rewritten or deleted), `store_facts_active(at)` no longer returns it, the untouched service-name fact still does, and bundle revision 2 quotes the service but not the same price revision. |
