...
| 4 | canonical handoff uses the server workspace and never crosses account or no-workspace boundaries | A 先取两个 membership，其后在最晚的第二 workspace 再添加 membership；真实交付后打开 canonical one-shot URL 必须对比 Core receipt workspace=最早的 server default；同一 document/QueryClient 内从产品 UI 退出 A、登录 B，再以 SPA 导航打开同 URL；必须产生新 `assisted_consume_handoff` server response（`consumed|not_found`），只显示 unavailable，不得命中 A 的 ready cache/文案。删除 B 的全部 membership 后再进同 URL 须直接 unavailable、不得发 consume 请求。 |
