...
| 3 | self-report refusals: same-day skip, once-per-work conflict, two-ignore backoff | 真实交付后走真实 P1 边界：同日 ask→`not_yet_next_day`；`mark_asked` 两次→409 `SELF_REPORT_ASK_CONFLICT`（once-per-work）；chip 对应的 OutcomeEvidence 落库为 merchant_recorded inquiry，回答后→`already_answered`；两个 work 连续 ignore→`store_backoff`。次日正向 chips UI 不由本浏览器用例声称（时钟可控正向路径在 Core）。 |
| 4 | canonical handoff uses the server workspace and never crosses account or no-workspace boundaries | A 先取两个 membership，其后在最晚的第二 workspace 再添加 membership；真实图文路径依次采用、下载完整发布包、在可见「本次发布确认」卡完成账号/时间/费用确认，且 exact export receipt、consumed approval、assisted delivery identity 都一致后才准备交接。打开 canonical one-shot URL 必须对比 Core receipt workspace=最早的 server default；从产品 UI 退出 A、登录 B 后（登录流可创建新 document）再以 SPA 导航打开同 URL；必须产生新 `assisted_consume_handoff` server response（`consumed|not_found`），只显示 unavailable，不得命中 A 的 ready cache/文案。长寿命 QueryClient 的跨 identity 隔离另由 `canonical-handoff-query.interaction.test.tsx` 覆盖。删除 B 的全部 membership 后再进同 URL 须直接 unavailable、不得发 consume 请求。 |
