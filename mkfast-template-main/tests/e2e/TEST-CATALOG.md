...
| # | Test name | Flow |
|---|---|---|
| 1 | A limited-time price is quoted inside its window and dropped once the clock passes it | Seed a legacy StoreProfile with one confirmed project, open the store wizard, and state a price together with the day it runs to — the save button stays disabled until that question is answered, so no price can reach the ledger without it. Read the sent `finalize_store_intake` and require the window to ride on the price candidate's own `expiresAt` and to be repeated identically on the profile side. Submit a customized copy journey and require the price revision to appear in the frozen ContextBundle's `referencedFactRevisions` and in the package's `factRefs`. Then move the merchant's clock (`page.clock`) to a day the server has long passed, restate the window from there so it closes behind the server's present, and require: the price stream still exists at revision 2 in `store_fact_history` (nothing is deleted), `store_facts_active` at the server's present no longer returns it, the untouched service-name fact still does, and a second customized journey freezes a ContextBundle that quotes the service and not the price. |
