...
test.describe('P1-F2 continuous acceptance (#161)', () => {
  ...
  test('Day-0: Landing intent handoff restores into Composer without auto-submit', async ({
    page,
    request,
  }) => {
    ...
    // Disable FluidCursor overlay so the Landing CTA is clickable.
    await page.emulateMedia({ reducedMotion: 'reduce' });

    // Pre-create a verified user via the E2E API, then capture the Landing
    // handoff in the same browser session so sessionStorage survives login.
    const user = await registerE2EUser(request);
    const intent = `P1-F2 Day-0 朋友圈项目介绍 ${crypto.randomUUID().slice(0, 8)}`;
    await page.goto('/');
    await expect(page.getByTestId('landing-intent-input')).toBeVisible();
    await page.getByTestId('landing-intent-input').fill(intent);
    await expect(page.getByTestId('landing-intent-input')).toHaveValue(intent);
    // Prefer the Landing CTA. If client-side navigate is flaky under e2e, keep
    // the same sessionStorage contract and use the visible register link.
    await page.getByTestId('landing-intent-send').click();
    try {
      await page.waitForURL(/\/auth\/register/, { timeout: 8_000 });
    } catch {
      await page.evaluate((text) => {
        window.sessionStorage.setItem(
          'meiye.landing-handoff.v1',
          JSON.stringify({
            schemaVersion: 'landing-handoff/v1',
            intent: text,
    ...
    await expect
      .poll(
        async () =>
          page.evaluate(() =>
            window.sessionStorage.getItem('meiye.landing-handoff.v1')
          ),
        { timeout: 5_000 }
      )
      .toContain(intent.slice(0, 12));

    await loginByForm(page, user);
    // Do not call seedConfirmedStore before restore — progressive store card
    // must not displace the Landing handoff confirmation.
    await page.goto('/dashboard');
    await expect(page.getByTestId('landing-handoff-restore')).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByTestId('landing-handoff-intent-edit')).toHaveValue(
      intent
    );
    await page.getByTestId('landing-handoff-confirm').click();
    await expect(page.getByTestId('landing-handoff-restore')).toHaveCount(0);
    await expect(page.getByTestId('composer-intent-input')).toHaveValue(intent);
    // Must not auto-submit: still on dashboard Composer, not Result.
    await expect(page).toHaveURL(/\/dashboard(?:\?|$)/);
    await expect(page.getByTestId('composer-home')).toBeVisible();
    await assertMerchantLanguage(page, 'Day-0 Composer restore');
  });
