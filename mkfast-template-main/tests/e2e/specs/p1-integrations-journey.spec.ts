...
test.describe('P1 integration product journeys', () => {
  ...
  test('runs strict BYOK and a published Feishu tool through recorded adapters', async ({
    ...
    await page.getByLabel('API Key（只写）').fill('recorded-byok-key');
    await page.getByLabel('授权范围').fill('model.invoke');
    ...
    await page.getByRole('tab', { name: '飞书 MCP' }).click();
    await page.getByLabel('飞书用户或应用标识').fill('E2E Feishu');
    await page.getByLabel('飞书 UAT（只写）').fill('recorded-feishu-uat');
    await page.getByLabel('授权范围').fill('mcp.tools, docx:read');
    await page.getByRole('button', { name: '创建连接' }).click();
