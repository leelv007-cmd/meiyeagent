...
async function createWork(page: Page, intent: string) {
  ...
  const confirmBrief = record.getByRole('button', { name: '采用并确认 Brief' });
...
test.describe('UI/UX Upgrade B durable video workflow', () => {
  ...
  test('a Work edits and restores V1, then the real fixture worker completes one durable video workflow', async ({
    ...
    const createDraftPayloads: Array<{
      executionContract?: { aspectRatio?: string; durationSeconds?: number };
      shots?: Array<{
    ...
    page.on('request', (browserRequest) => {
      ...
      if (
        body?.module === 'model-supply' &&
        body.action === 'video_workflow_create_draft'
      ) {
        createDraftPayloads.push(body.payload ?? {});
      }
    });
    ...
    expect(createDraftPayloads).toHaveLength(2);
    for (const payload of createDraftPayloads) {
      expect(payload.executionContract).toMatchObject({
        aspectRatio: '9:16',
        durationSeconds: 15,
      });
      expect(payload.shots?.map((shot) => shot.durationSeconds)).toEqual([
        4, 4, 4, 3,
      ]);
      expect(
        payload.shots?.every(
          (shot) => shot.height === 1280 && shot.width === 720
        )
      ).toBe(true);
    }
    expect(
